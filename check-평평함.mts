...
/* ⚠ `t-card` 는 «카드»가 아니라 «글자 크기» 이름이다 (2026-09-01).
 *   t-page · t-sec · t-sub · t-card 는 한 식구다 — 열여섯 팩에서 4,395번 쓰는데
 *   3,078번이 <h3> 에 붙는다. 카드 «제목»에 쓰라고 지은 이름이지 카드 상자가 아니다.
 *   그런데 낱말 끝(b)이 「t-card」의 뒷동강을 물어 카드로 세고 있었다.
 *   반려동물케어 프리미엄 PL0103(견종 자동완성 검색)에서 처음 드러났다 —
 *   견종 20줄이 나란한 목록인데 「카드 20개가 줄지어 있다」로 잡혔다.
 *   찾기 칸과 거르는 칩이 달린 검색 목록이라 20줄인 것이 «맞다».
 *   표·아코디언을 애초에 안 보는 것과 같은 까닭으로 뺀다. */
const 카드인가 = (c: string) => /\b(card|mcard|scard|item|tile|prod|goods)\b/.test(c) && !/\bacc-/.test(c) && !/\bt-card\b/.test(c);
