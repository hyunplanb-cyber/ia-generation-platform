 ...
 *
 * 자리 (2026-08-13 에 «_이미지» 한 곳으로 모았다)
 *   판매용_템플릿/_이미지/사진/<업종>/*.jpg|png|webp   ← 받은 사진을 여기에
 *   판매용_템플릿/_이미지/사진/<업종>/출처.csv          ← 어디서 받았는지
 *   판매용_템플릿/_이미지/고양이/장면/*.png             ← 팩에 쓰는 고양이·동물 (배경 있는 것)
...
/* ⚠ 2026-08-13 에 그림을 «_이미지» 한 곳으로 모았다.
   ...
   `고양이` 라고 부르면 _이미지/고양이/장면 을 원본으로 본다.

   ⛔ 2026-09-02 에 자리를 갈랐다 (사장님 지시) — 마스코트는 «긴머리»다.
     고양이·동물은 팩에 쓰고, 긴머리는 SNS 에 쓴다. 그래서 원본 자리도 나뉜다.
     옛 이름 `마스코트` 로 불러도 받아 준다 — 대본·문서가 아직 그렇게 부른다. */
const 원본방 = "판매용_템플릿/_이미지/사진";
const 낼방뿌리 = "판매용_템플릿/_이미지/팩용";
const 고양이원본 = "판매용_템플릿/_이미지/고양이/장면";
/** 옛 이름도 받아 준다 — 「마스코트」라고 불러도 고양이 자리를 본다. */
const 고양이인가 = (업종: string) => 업종 === "고양이" || 업종 === "마스코트";
const 원본어디 = (업종: string) => (고양이인가(업종) ? 고양이원본 : join(원본방, 업종));
...
const 업종들 = 고른업종 && 고양이인가(고른업종)
  ? ["고양이"]
  : readdirSync(원본방, { withFileTypes: true })
