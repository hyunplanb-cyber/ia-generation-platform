...
function checkPack(dir: string) {
  ...
  const rules: [string, string][] = [
    ...
    // 파일을 어디 두고 어떻게 부를지가 없어서 CSS 가 통째로 안 붙은 적이 있다.
    // 저장소 안에서 만들면 옆 사이트를 보고 따라 해 안 깨지지만, 빈 폴더에서는 깨진다(2026-08-07).
    ["파일을 어디에 두나", "폴더 구조 안내"],
    ["../assets/css/style.css", "상대 경로 안내"],
    ["토스트로 때우지 마세요", "안내로 때우지 말 것"],
  ];
  for (const [needle, label] of rules) {
    // 라벨 끝 글자에 받침이 있으면 "이", 없으면 "가".
    if (!md.includes(needle)) {
      const last = label.charCodeAt(label.length - 1);
      const 받침 = last >= 0xac00 && last <= 0xd7a3 && (last - 0xac00) % 28 !== 0;
      add(dir, "막음", `빌드 가이드에 ${label}${받침 ? "이" : "가"} 없음`);
    }
  }
