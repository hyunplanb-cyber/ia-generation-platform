...
■ 만든 뒤에는 꼭 눌러 보세요
 ...
 AI는 실수합니다. 스펙을 잘 써 두어도 만들어진 화면이 의도와 다를 수 있습니다.
 이 AI팩을 만든 저희도, 같은 설계로 화면을 만들 때마다 고칠 것이 나왔습니다.

 실제로 나왔던 것들입니다 —
  · 뒤로가기가 화면에 안 붙는다 (2뎁스 이상 125개 중 125개 전부 빠진 적이 있습니다)
  · 정의하지 않은 CSS 값을 써서 간격이 조용히 0이 된다
  · 목록 카드가 눌러도 아무 일이 없다
  · 탭인데 어떤 화면엔 뒤로가기가 있고 어떤 화면엔 없다
  · 배지·버튼 색이 배경과 같아 강조가 강조로 안 보인다
  · 세로로 쌓은 버튼끼리 간격이 0이라 달라붙는다
  · 화면 안에서 끝나야 할 조작이 '준비 중입니다' 안내로 때워진다

 스펙이 부실해서가 아닙니다. AI가 스펙을 읽고 화면으로 옮기는 사이에 어긋납니다.
 그래서 만들고 끝내지 마시고, 아래 두 가지로 확인해 주세요.
 ...
 빠르게 훑으면 30분, 꼼꼼히 보면 두세 시간입니다. 오픈 전에 꼭 한 번 하세요.
 다른 것이 있으면 그 화면의 '생성 프롬프트'만 다시 넣어 고치시면 됩니다.
