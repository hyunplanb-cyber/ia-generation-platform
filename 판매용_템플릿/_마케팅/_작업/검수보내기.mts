 ...
 *
 *   ⭐ 새 2분할 틀로 구웠으면 «반드시» --새틀 을 붙인다:
 *     … <대본.json> <회차> --새틀 "<회차폴더 이름>"
 *     안 붙이면 옛 통짜 사슬이 구운 것을 찾다가 못 찾거나, 더 나쁘게는
 *     «옛 틀로 구운 묵은 파일»을 그대로 올린다 (2026-09-11 에 그랬다).
...
/* 회차 폴더가 어디 있나 — 규칙은 `릴스영상/_회차방.mjs` 한 곳이 정한다 */
const { 회차방길 } = await import("../릴스영상/_회차방.mjs");
...
/* ⭐ 새 2분할 사슬로 구운 것을 보낸다 (2026-09-11).
 *   `릴스영상/영상만들기.mjs` 는 «회차 폴더»에 최종_9x16.mp4 · 최종_16x9.mp4 를 낸다.
 *   옛 통짜 사슬(`_작업/영상굽기.mjs`)은 %TEMP%/cc-vid-w2 에 {이름}_916.mp4 를 낸다.
 *   두 자리가 다르고, 틀도 다르다(_규격.mjs ↔ 영상틀_916.html).
 *   ⛔ 이것을 안 이어 두어 9/11 에 세 편이 옛 틀로 나갔다. 현님이 보고 잡으셨다.
 *
 *   쓰는 법:  npx tsx 검수보내기.mts <대본.json> <회차> --새틀 "<회차폴더 이름>"
 *   새 사슬에는 «인트로가 따로 없다» — 제목 띠가 그 노릇을 한다. */
const 새틀회차 = (() => {
  const i = process.argv.indexOf("--새틀");
...
for (const 편 of 대본들) {
  ...
  /* 새 사슬이면 회차 폴더의 최종_9x16.mp4 를 쓴다 */
  const 회차방 = 새틀회차 ? 회차방길(join(마케팅, "릴스영상"), 새틀회차) : null;
  const 영상 = 회차방 ? join(회차방, "최종_9x16.mp4") : join(구운방, `${이름}_916.mp4`);
  if (!캡션만 && !existsSync(영상)) {
    console.error(`\n❌ 구운 세로 영상이 없습니다: ${영상}`);
    console.error(새틀회차
      ? "   먼저 릴스영상/영상만들기.mjs 를 돌리세요 — 실제로 나갈 프레임을 뽑아야 합니다."
      : "   먼저 영상굽기.mjs 를 돌리세요 — 실제로 나갈 프레임을 뽑아야 합니다.");
    process.exit(1);
  }
  ...
  if (!캡션만) {
    ...
    /* ⚠ 자를 사슬에 맞춘다. 새 사슬을 옛 틀에 대면 «언제나 새것»이라 묵은 것을 못 잡는다.
       새 2분할 틀의 좌표는 `릴스영상/_규격.mjs` 가 정하고 `틀그리기.mjs` 가 PNG 를 낸다. */
    const 본편틀 = 새틀회차
      ? Math.max(때(join(마케팅, "릴스영상/_규격.mjs")), 때(join(마케팅, "릴스영상/틀그리기.mjs")))
      : Math.max(틀때("영상틀_916.html"), 틀때("영상틀_169.html"));
    const 커버틀 = 틀때("썸네일틀.html");
    const 가로 = 회차방 ? join(회차방, "최종_16x9.mp4") : join(구운방, `${이름}_169.mp4`);
    /* 새 사슬엔 인트로가 «따로 없다» — 제목 띠가 그 노릇을 한다 */
    const 인트로 = 새틀회차 ? undefined : 인트로설정(이름)?.낼길;
...
❌ ${이름}: 묵은 것이 섞여 있습니다.`);
      ...
      console.error(새틀회차
        ? "   릴스영상/틀그리기.mjs → 자막굽기.mjs → 영상만들기.mjs 를 다시 돌린 뒤에 보냅니다."
        : "   영상굽기.mjs → 영상인트로.mjs 를 «둘 다» 다시 돌린 뒤에 보냅니다.");
      process.exit(1);
    }
  }
  ...
  try {
    ...
    if (캡션만) {
      ...
      /* ⭐ 새 2분할 사슬엔 «인트로가 없다» — 제목 띠가 그 노릇을 한다 (2026-09-11).
       *   옛 사슬은 2초짜리 인트로를 따로 구워 그 1초 지점을 커버로 썼다.
       *   새 사슬은 첫 프레임부터 제목 띠와 마스코트가 이미 들어 있다 —
       *   «보는 사람이 처음 보는 장면»이 곧 커버다. 그래서 본편에서 뜬다.
       *   ⛔ 이걸 안 이어 두면 새 사슬 편은 커버 없이 나간다(9/11 에 넷이 그랬다). */
      const 낼길 = join(임시, "cover.webp");
      execFileSync("ffmpeg", [
        "-v", "error", "-ss", "1", "-i", 영상, "-frames:v", "1",
        "-vf", "scale=405:-1", "-q:v", "72", 낼길, "-y",
      ]);
      커버 = `data:image/webp;base64,${readFileSync(낼길).toString("base64")}`;
      console.log(`   커버 1장 (${Math.round(커버.length / 1024 * 0.75)}KB) · 본편 1초 지점 (새 사슬은 인트로가 없습니다)`);
    } else if (!설정) {
    ...
    const 값 = {
      ...
      /* 새 사슬은 인트로설정이 없다 — 커버 글은 «상단 띠 제목» 그 자체다 */
      coverTitle: 설정?.title ?? (새틀회차 ? (편.세로제목 ?? "") : ""),
      coverSub: 설정?.cap ?? "",
      /* ⭐ 검수 화면이 «영상을 바로 돌려 보게» 경로를 적어 둔다. 커버 붙인 것이 있으면 그것을 쓴다. */
      videoVertical: (인트로영상 && existsSync(인트로영상)) ? 인트로영상 : 영상,
      videoHorizontal: 회차방 ? join(회차방, "최종_16x9.mp4") : join(구운방, `${이름}_169.mp4`),
