...
for (const t of 목록) {
  ...
  /* ⭐ 커버 마스코트는 «하단 우측» 고정이다 (2026-08-20 사장님: 「하단 우측에 고정해줘」).
     tr(우측 위)로 두었더니 제목을 가렸고, 자리를 옮길 때마다 제목이나 목록 중 하나를
     물었다. 자리는 이제 한 곳이다.
     ⛔ 그래서 인트로설정의 pos 칸도 지웠다 (2026-08-26 사장님 지시). 안 보는 값이 파일에
        남아 있으면 다음 사람이 그것을 고치고 왜 안 바뀌는지 한참 찾는다.
        다시 자리를 고르게 하려면 이 줄의 "br" 을 t.pos 로 되돌리고 설정에 칸을 되살린다. */
