...
for (const t of 목록) {
  ...
  /* ⭐ 커버 마스코트를 «본편에 쓰인 마스코트 영상»과 짝지운다 (2026-08-20 사장님).
     「내용쪽에 들어간 영상의 캐릭터… 커버에 이 아이들이 노출되는 것이 좋을거 같아」
     _최근.json 의 맨 앞이 이번 편에 쓴 영상이다. 그 장면과 같은 결의 누끼 PNG 를 쓴다.
     짝이 없으면 설정에 적힌 pose 를 그대로 쓴다. */
  let 고른포즈 = t.pose;
  try {
    const 짝 = JSON.parse(readFileSync(`${마케팅}/_작업/_커버짝.json`, "utf8"));
    const 편별 = JSON.parse(readFileSync(`${마케팅}/../_이미지/마스코트/영상/_편별.json`, "utf8"));
    const 이번영상 = 편별[t.이름];
    if (이번영상 && 짝[이번영상]) {
      고른포즈 = 짝[이번영상];
      console.log(`  커버 마스코트 ← 본편 영상(${이번영상}) 과 짝: ${고른포즈}`);
    }
  } catch { /* 짝표가 없으면 설정대로 간다 */ }
  const 포즈길 = 포즈파일(고른포즈);
  if (!existsSync(포즈길)) throw new Error(`포즈가 없습니다: ${고른포즈}`);
  ...
  const 공통 = { v: 9, big: t.title, sub: t.cap, ep: t.ep, face: "cat",
    pos: t.pos ?? "br", sheet: t.sheet, rows: t.rows };
  ...
  console.log(`  고양이 ${t.pose} → ${자.w}×${자.h} @ (${자.x}, ${자.y}) · ±${흔들폭}px`);
