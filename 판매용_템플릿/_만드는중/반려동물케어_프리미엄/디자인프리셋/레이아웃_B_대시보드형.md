...
| 자리 | 어떻게 |
| --- | --- |
| 첫 화면 위쪽 | 히어로 없음. 화면 위쪽에 지표 카드 4개. |
| 목록 화면 | 표 중심. 헤더 고정, 행 높이 일정, 상태는 배지로. |
| 내비게이션 | 좌측 세로 사이드바(그룹 제목 + 항목) + 상단에는 계정만. |
| 상세 화면 | 좌 본문 + 우 액션 패널 2단. 상태 변경 버튼을 우측에 모은다. |
| 카드(썸네일) | 사진을 쓰지 않는다. 제목·숫자·배지만으로 위계를 만든다. 좌측에 색 띠 한 줄. (비율 없음) |

**어울리는 곳** — 관리자·백오피스·사장님 화면
...
이 뼈대의 기본 카드는 **사진 없는 카드**입니다. 아래 중 다른 것으로 바꿔 쓰셔도 됩니다 — 뼈대는 그대로 두고 카드만 갈아 끼우면 인상이 크게 달라집니다.
...
| 사진 없는 카드 **(기본)** | 없음 | 사진을 쓰지 않는다. 제목·숫자·배지만으로 위계를 만든다. 좌측에 색 띠 한 줄. | 표·대시보드처럼 숫자가 주인공인 화면 |
...
- 내비게이션: 좌측 세로 사이드바(그룹 제목 + 항목) + 상단에는 계정만.
- 상세 화면: 좌 본문 + 우 액션 패널 2단. 상태 변경 버튼을 우측에 모은다.
- 카드(썸네일): 사진을 쓰지 않는다. 제목·숫자·배지만으로 위계를 만든다. 좌측에 색 띠 한 줄. 이미지 비율은 없음 를 지켜줘.
