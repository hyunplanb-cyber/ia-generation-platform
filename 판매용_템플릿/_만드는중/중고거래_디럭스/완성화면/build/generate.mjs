/* 07_AI빌드_스펙팩.json 의 2뎁스 화면(디럭스 · 40화면)을 정적 HTML 로 생성한다.
...
/* ⚠ 프리미엄은 디럭스와 «일부러» 다른 프리셋을 쓴다 —
   디럭스(이 팩)는 코럴 선셋 + 목록 중심형, 프리미엄은 레트로 페이퍼 + 대시보드형이다. */
...
const LAYOUT = 찾기('레이아웃 프리셋', [
  path.resolve(ROOT, '스펙팩/레이아웃_A_목록중심형.json'),
  path.resolve(ROOT, '../디자인프리셋/레이아웃_A_목록중심형.json'),
]);
...
<main class="main"><div class="wrap">
  <div class="card mb8"><div class="card-bd">
    <h2 class="t-card mb3">디자인 프리셋 — 가이드 01 ${preset.preset.name} × 레이아웃 A ${layout.layout.name}</h2>
    ...
    <p class="t-sub mt2">레이아웃 A ${layout.layout.name} — <b>히어로를 두지 않습니다.</b>
    제목과 필터 칩 줄로 바로 시작하고, 목록 화면에서만 왼쪽에 필터 사이드바가 붙습니다.
    상세는 상단 요약 바 + 탭으로 나눈 본문이고, 운영자 화면 5쪽은 그 안에 제 메뉴를 하나 더 두어 손님 화면과 갈립니다.</p>
    <p class="t-sub mt2">카드 모양은 <b>가로 행(row)</b> — 왼쪽 정사각 썸네일, 가운데 제목과 동네·시각, 오른쪽 값과 찜.
    물건 사진은 품질도 비율도 제각각이라 썸네일 자리를 정사각으로 못 박아 모든 줄에서 같은 크기를 씁니다.</p>
