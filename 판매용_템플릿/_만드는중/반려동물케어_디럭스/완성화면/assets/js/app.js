...
(function () {
  ...
  function 미체크세기() {
    ...
    if (!칸) return;
    var n = 모두('.pc-check[data-row]').filter(function (r) {
    ...
    if (말) 말.textContent = n === 0
      ? (잠김 ? '남은 ' + 잠김 + '마리는 백신 확인이 필요해요' : '오늘 올 아이는 다 왔어요')
      : '아직 ' + n + '마리를 기다리고 있어요';
  }
  ...
  document.addEventListener('input', function (e) {
    ...
    모두('[data-search-word="' + key + '"]').forEach(function (x) { x.textContent = 말 || '—'; });
  });
  ...
  document.addEventListener('input', function (e) {
    ...
    var 반 = kg < 5 ? ['소형반', '5kg 미만'] : (kg < 15 ? ['중형반', '5~15kg'] : ['대형반', '15kg 이상']);
