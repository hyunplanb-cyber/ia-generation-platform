...
import {
  ...
  field, input, select, textarea, check, toggle, radioRow, uploadDrop, link, 조사,
} from './ui.mjs';
import {
...
${banner('info', '💬', `<b>원하는 답을 못 찾으셨나요?</b>
  ...
  급하시면 카카오톡 채널 ${esc(SITE.kakao)}로 남겨 주세요.</div>`,
...
<div class="mt8" style="font-size:var(--fs-card);line-height:var(--lh-body)">
  ...
  <p>카카오톡 채널 <b>${esc(SITE.kakao)}</b>로 남겨 주세요. 연휴에도 하루 한 번 확인합니다.
...
${card('첨부파일', `<div class="stack">
  ${[['추석 연휴 휴무 안내문.pdf', '182KB'], ['9월 등원 달력.pdf', '96KB']].map(([nm, sz]) => `<div class="row-b wrap-row">
    ...
    ${btn('내려받기', { cls: 'btn-ghost', sm: true, attr: ` data-toast="${esc(nm)} 을(를) 내려받습니다"` })}
  </div>`).join('')}
</div>`, { cls: 'mt8' })}
...
<div class="row-b wrap-row mt8" style="border-top:1px solid var(--border);padding-top:var(--sp-block)">
  ${btn(`‹ 이전 · ${esc(앞.t.length > 16 ? 앞.t.slice(0, 16) + '…' : 앞.t)}`, { href: 'CS-03', cls: 'btn-ghost', sm: true })}
  ${btn(`다음 · ${esc(뒤.t.length > 16 ? 뒤.t.slice(0, 16) + '…' : 뒤.t)} ›`, { href: 'CS-03', cls: 'btn-ghost', sm: true })}
</div>
