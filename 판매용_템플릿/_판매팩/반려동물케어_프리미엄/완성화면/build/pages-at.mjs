...
import {
  SITE, TODAY, ALERTS, DOGS, DOG, CLASSES, CLS, clsNow, inClass, TODAY_STAT, CAME,
} from './data.mjs';
...
/** 오늘 그 아이 보호자께 «실제로» 연락한 기록이 있으면 그것을 적는다.
    ⛔ 「연락 시도 없음」을 붙박이로 적으면 ALERTS 에 기록이 있는 아이에게도 없다고 말한다
       — 같은 사건을 두 화면이 다르게 말하는 자리다(2026-08-31). */
function 연락기록(d, 표 = false) {
  const 오늘 = `${String(TODAY.m).padStart(2, '0')}-${String(TODAY.d).padStart(2, '0')}`;
  const a = ALERTS.find((x) => x.dog === d.nm && String(x.when).startsWith(오늘));
  if (!a) return 표 ? '<span class="muted">아직 없음</span>' : '연락 시도 없음';
  const 시각 = String(a.when).slice(6);
  return 표
    ? `${esc(a.ch)} ${esc(a.st)}<div class="sub">${esc(시각)}</div>`
    : `${esc(시각)} ${esc(a.ch)} ${esc(a.st)}`;
}

/** 예약 시각과 알려온 도착 예정 시각의 차이를 «셈해서» 적는다.
    ⛔ 「80분 늦음」처럼 손으로 적으면 시각을 고쳐도 이 배지만 안 따라온다. */
function 늦은만큼(예약, 도착) {
  const 분 = (t) => { const [h, m] = String(t).split(':').map(Number); return h * 60 + m; };
  const 차 = 분(도착) - 분(예약);
  if (차 <= 0) return '늦지 않음';
  const h = Math.floor(차 / 60), m = 차 % 60;
  return h ? `${h}시간 ${m ? `${m}분 ` : ''}늦음` : `${m}분 늦음`;
}
...
${미등원.length ? sec('먼저 확인해 주세요', 미등원.map((d) => `<div class="rowcard bad mb3">
  ...
  <div class="bd">
    ...
    <div class="t-sub mt1">보호자 ${esc(d.guardian)} · ${esc(d.phone || '연락처 없음')} · ${연락기록(d)}</div>
  </div>
...
<div class="g3">
  ${stat('아직 안 온 아이', `<span data-untick>${체크할것.length}</span>`   /* ⚠ 잠긴 줄(백신 확인 대기)도 «아직 안 온 아이»다 — 빼고 세면 명단 5줄에 숫자 4가 되고, 옆 칸의 「오늘 예약 24마리 중」과도 안 맞는다 (2026-09-01) */, { ico: '⏳', u: '마리', d: '<span data-untick-msg>등원 체크를 기다리고 있어요</span>' })}
    ...
    const 지각칸 = `
      ${지각.length ? table(
        ...
        지각.map((d) => [
          ...
          { t: badge(늦은만큼(d.want, d.eta), 'b-warn'), cls: 'nowrap' },
