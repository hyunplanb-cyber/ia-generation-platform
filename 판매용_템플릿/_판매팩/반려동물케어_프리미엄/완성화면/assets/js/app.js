...
(function () {
  ...
  if (!지금) return;
  var 갈래 = 지금.slice(0, 2);
  /* ⛔ 2026-09-02: .gnb-nav · .own-side 만 찾고 있었다. 이 팩의 옆 차림표는 «nav.appside» 다
     (.gnb-nav 도 .own-side 도 한 장에도 없다). 그래서 «지금 쪽» 이 한 번도 안 켜졌다.
     다른 팩에서 옮겨 올 때를 대비해 옛 이름도 같이 둔다. */
  var 칸들 = document.querySelectorAll('.appside, .side, .gnb-nav, .own-side');
