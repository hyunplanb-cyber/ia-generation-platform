...
| 자리 | 어떻게 |
| --- | --- |
| 첫 화면 위쪽 | 어두운 배경 위에 화면 폭을 꽉 채운 큰 제목(48px 이상). 바로 아래 숫자 지표 4개를 가로 한 줄로. |
| 목록 화면 | 카드를 나란히 두지 않고 위아래로 조금씩 어긋나게(계단) 배치. 카드마다 큰 숫자를 하나씩 얹는다. |
| 내비게이션 | 상단 가로 GNB. 어두운 배경에 흰 글자, 오른쪽 끝에 밝은 버튼 하나. |
| 상세 화면 | 상단 어두운 띠에 제목과 핵심 숫자 → 아래는 밝은 배경 본문. 액션은 하단 고정 바. |
| 카드(썸네일) | 사진이 카드 위쪽을 정사각으로 채우고, 아래에 제목 → 보조정보 → 가격. (비율 1:1) |

**어울리는 곳** — 숫자가 설득하는 것(공동구매·펀딩·챌린지·부트캠프)
...
| 1024px (태블릿 가로) | 24px | 16px | 3칸 | 314px |
| 375px (휴대폰) | 16px | 12px | 2칸 | 165px |

카드 사진은 **1:1**입니다. 위 폭에 맞추면 사진 크기는 이렇게 됩니다.

- 노트북 336×336 · 태블릿 314×314 · 휴대폰 165×165
