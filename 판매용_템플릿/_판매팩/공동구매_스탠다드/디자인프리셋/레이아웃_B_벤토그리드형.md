...
| 자리 | 어떻게 |
| --- | --- |
| 첫 화면 위쪽 | 히어로를 따로 두지 않는다. 첫 화면부터 크기가 다른 블록을 한 판에 짠다 — 큰 블록 1개(대표)에 작은 블록 3~4개를 붙인다. |
| 목록 화면 | 블록 크기를 2:1:1로 섞는다. 첫 항목만 두 칸을 차지하고 나머지는 한 칸씩. 칸 사이 간격은 일정하게. |
| 내비게이션 | 상단 가로 GNB 한 줄. 블록이 이미 복잡하므로 내비는 최대한 단순하게. |
| 상세 화면 | 위에 큰 블록으로 핵심 정보, 아래에 작은 블록으로 세부. 액션은 큰 블록 안에 둔다. |
| 카드(썸네일) | 사진이 카드 위쪽을 정사각으로 채우고, 아래에 제목 → 보조정보 → 가격. (비율 1:1) |

**어울리는 곳** — 기능이 여럿인 것(서비스 소개·기능 안내·요약 대시보드)

## 칸이 몇 개이고, 카드가 몇 px인가

| 화면 폭 | 좌우 여백 | 칸 사이 | 칸 수 | 카드 한 장 |
| --- | --- | --- | --- | --- |
| 1440px 이상 (노트북) | 24px | 16px | 4칸 | 336px |
| 1024px (태블릿 가로) | 24px | 16px | 2칸 | 480px |
| 375px (휴대폰) | 16px | 12px | 1칸 | 343px |

카드 사진은 **1:1**입니다. 위 폭에 맞추면 사진 크기는 이렇게 됩니다.

- 노트북 336×336 · 태블릿 480×480 · 휴대폰 343×343
...
```css
.cards { grid-template-columns: repeat(4, 1fr); grid-auto-flow: dense; }
...
이 뼈대의 기본 카드는 **정사각 카드**입니다. 아래 중 다른 것으로 바꿔 쓰셔도 됩니다 — 뼈대는 그대로 두고 카드만 갈아 끼우면 인상이 크게 달라집니다.

| 카드 모양 | 비율 | 어떻게 | 어울리는 곳 |
| --- | --- | --- | --- |
| 정사각 카드 **(기본)** | 1:1 | 사진이 카드 위쪽을 정사각으로 채우고, 아래에 제목 → 보조정보 → 가격. | 상품·공구처럼 물건 자체를 보여줄 때 |
...
| 사진 위 겹침 | 4:3 | 사진 위에 어두운 그라데이션을 깔고 제목·태그를 그 위에 얹는다. 카드 밖에 글자가 없다. | 여행지·매장처럼 사진 한 장으로 설명되는 것 |
...
- 내비게이션: 상단 가로 GNB 한 줄. 블록이 이미 복잡하므로 내비는 최대한 단순하게.
- 상세 화면: 위에 큰 블록으로 핵심 정보, 아래에 작은 블록으로 세부. 액션은 큰 블록 안에 둔다.
- 카드(썸네일): 사진이 카드 위쪽을 정사각으로 채우고, 아래에 제목 → 보조정보 → 가격. 이미지 비율은 1:1 를 지켜줘.
