...
| 자리 | 어떻게 |
| --- | --- |
| 첫 화면 위쪽 | 히어로를 두지 않는다. 제목 + 필터 칩 줄로 바로 시작. |
| 목록 화면 | 좌측 필터 패널 + 우측 가로 리스트 행(썸네일 왼쪽, 정보 오른쪽, 액션 끝). |
| 내비게이션 | 상단 가로 GNB + 목록 화면에서만 좌측 필터 사이드바. |
| 상세 화면 | 상단 요약 바 + 탭으로 나눈 본문. |
| 카드(썸네일) | 왼쪽에 작은 정사각 썸네일, 오른쪽에 제목·정보, 맨 끝에 액션 버튼. (비율 1:1 (왼쪽 고정)) |

**어울리는 곳** — 비교해서 고르는 서비스(중개·구인·상품 비교)

## 칸이 몇 개이고, 카드가 몇 px인가

| 화면 폭 | 좌우 여백 | 칸 사이 | 칸 수 | 카드 한 장 |
| --- | --- | --- | --- | --- |
| 1440px 이상 (노트북) | 24px | 16px | 3칸 | 356px |
| 1024px (태블릿 가로) | 24px | 16px | 2칸 | 358px |
| 375px (휴대폰) | 16px | 12px | 1칸 | 343px |

카드 사진은 **1:1 (왼쪽 고정)**입니다. 위 폭에 맞추면 사진 크기는 이렇게 됩니다.

- 노트북 356×356 · 태블릿 358×358 · 휴대폰 343×343
...
```css
/* 사이드바는 폭을 고정한다 — 비율(fr)로 두면 화면이 넓어질수록 같이 넓어져 빈다 */
...
```css
.card.row { background: none; border: 0; box-shadow: none;
...
이 뼈대의 기본 카드는 **가로 행**입니다. 아래 중 다른 것으로 바꿔 쓰셔도 됩니다 — 뼈대는 그대로 두고 카드만 갈아 끼우면 인상이 크게 달라집니다.
...
| 가로 행 **(기본)** | 1:1 (왼쪽 고정) | 왼쪽에 작은 정사각 썸네일, 오른쪽에 제목·정보, 맨 끝에 액션 버튼. | 비교하며 훑는 목록 |
...
- 내비게이션: 상단 가로 GNB + 목록 화면에서만 좌측 필터 사이드바.
- 상세 화면: 상단 요약 바 + 탭으로 나눈 본문.
- 카드(썸네일): 왼쪽에 작은 정사각 썸네일, 오른쪽에 제목·정보, 맨 끝에 액션 버튼. 이미지 비율은 1:1 (왼쪽 고정) 를 지켜줘.
