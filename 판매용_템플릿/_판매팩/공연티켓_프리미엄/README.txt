...
■ 구성
 ...
 02_IA_화면목록.xlsx     화면 228개 + 화면별 AI 생성 프롬프트  ★핵심
 03_기능정의서.xlsx      요건 1262개 (업무·기능·구성·유형)
 ...
 08_검수시나리오.xlsx     오픈 전 점검표 — 시나리오 228개  ★디럭스·프리미엄
 디자인프리셋/
...
■ 어디까지 만들어지나요 — 먼저 읽어주세요
 ...
 이 서비스에서 개발자에게 넘길 항목 6가지
  · 좌석 관리 — «몇 장 남았나»가 아니라 «어느 자리가 남았나» — 구역·열·번 단위 재고
  · 회차·캐스팅 — 한 공연에 수십 회차, 회차마다 다른 캐스팅이 사실상 다른 상품
  · 결제·취소 — 취소 시점별 수수료 단계, 부분 취소, 우천 일괄취소(수수료 0원)
  · QR 검표 — 현장에서 폰으로 쓰는 검표와 오프라인 저장 후 전송
  · 정산 — 판매수수료·카드수수료·예매수수료가 각각 다른 주체의 몫
  · 알림 — 예매 확인·공연 하루 전·회차 변경·환불 완료 문자와 알림톡
