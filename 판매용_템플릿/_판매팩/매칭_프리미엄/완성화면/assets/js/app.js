...
(function () {
  ...
  /* ── HO-01-03 지역 고르기 — 고른 시·도·구·군이 아래 버튼 글로 이어진다 (2026-09-02)
   *   경기·마포구를 골라도 「서울 강남구로 보기」 그대로였다. 고른 것이 뒤로 안 이어지는 것이라
   *   검수항목 G4 다. 칩손질() 이 켠 «뒤»에 읽어야 하므로 거품 단계에서 듣는다. */
  (function 지역이음() {
    var 낼칸 = document.querySelector('[data-region-out]');
    if (!낼칸) return;
    function 다시쓰기() {
      var 말 = ['시도', '구군'].map(function (k) {
        var 켜진 = document.querySelector('[data-region="' + k + '"] .chip.on');
        return 켜진 ? (켜진.textContent || '').replace(/[✕✖×]/g, '').trim() : '';
      }).filter(Boolean).join(' ');
      /* ⚠ 고른 것이 없으면 「전국로 보기」가 된다 — 그때는 옆 단추와 같은 말이 되므로
         「전국으」로 적어 뒤의 「로 보기」와 이어 붙인다. */
      낼칸.textContent = 말 || '전국으';
    }
    document.addEventListener('click', function (e) {
      if (e.target && e.target.closest && e.target.closest('[data-region]')) setTimeout(다시쓰기, 0);
      /* ⚠ capture 로 듣는다 — 칩손질() 이 stopPropagation 을 부르므로 거품 단계까지 안 온다.
         같은 element 의 capture 손잡이는 뒤에 붙어도 부른다(stopImmediatePropagation 이 아니라서). */
    }, true);
    다시쓰기();
  })();
