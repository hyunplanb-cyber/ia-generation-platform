...
/* 프리미엄은 디럭스와 «일부러» 다른 프리셋을 쓴다 —
   디럭스는 모던 네이비 + 좌우 분할형, 여기는 미니멀 모노 + 목록 중심형이다. */
...
/* 2뎁스 빌더는 'HO-01' 로 적혀 있고 3뎁스 스펙팩은 'HO0101' 로 부른다.
   같은 화면이라 이름만 맞춰 준다 — 두 번째 01 은 「그 화면의 기본 상태」다. */
...
<main class="main"><div class="wrap">
  <div class="card mb8"><div class="card-bd">
    <h2 class="t-card mb3">디자인 프리셋 — 가이드 02 미니멀 모노 × 레이아웃 B 목록 중심형</h2>
    ...
    <p class="t-sub mt2">레이아웃 B — <b>메뉴가 왼쪽 세로 바</b>에 붙어 어느 화면에서나 보입니다.
    히어로는 좌우 2단(왼쪽 글, 오른쪽 요청서 카드), 목록은 좌측 필터 + 우측 목록,
    상세는 본문 한 단(최대 760px) + 하단 고정 액션 바.
    고수가 쓰는 화면 47쪽은 그 안에 제 메뉴를 하나 더 두어 손님 화면과 갈립니다</p>
  ...
  <div class="box mt8">
    ...
    <p class="t-sub">스펙팩(07_AI빌드_스펙팩.json)의 화면 정의에 디자인 프리셋 두 벌(가이드 02 미니멀 모노 · 레이아웃 B 목록 중심형)을 함께 넣어 만든 정적 프로토타입입니다.
    화면마다 스펙팩에 적힌 화면 프롬프트를 그대로 따라 만들었습니다. 2뎁스 43화면만 담은 디럭스 등급도 있습니다.
