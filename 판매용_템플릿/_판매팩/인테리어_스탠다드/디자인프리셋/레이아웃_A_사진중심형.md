...
| 자리 | 어떻게 |
| --- | --- |
| 첫 화면 위쪽 | 검색바 없이 사진 모자이크(큰 1장 + 작은 4장). 문구는 사진 아래에 겹치지 않게. |
| 목록 화면 | 3열(1440px에서 카드 453px). 큰 이미지 위에 제목·태그를 얹고 설명은 짧게. 1024px에서 2열, 720px 아래로는 1열. |
| 내비게이션 | 상단 가로 GNB + 그 아래 카테고리 줄(2단 헤더). |
| 상세 화면 | 상단 전체 폭 갤러리 → 아래 정보. 액션은 하단 고정 바. |
| 카드(썸네일) | 사진 위에 어두운 그라데이션을 깔고 제목·태그를 그 위에 얹는다. 카드 밖에 글자가 없다. (비율 4:3) |

**어울리는 곳** — 보이는 게 중요한 서비스(뷰티·인테리어·포트폴리오·여행)
...
- 노트북 453×340 · 태블릿 480×360 · 휴대폰 343×257
