...
export const PAGES = {
  /* ============================================================
     ...
     ============================================================ */
  'NW-01': () => {
    /* ⚠ 아래 안내가 「가장 먼저 하원한 아이가 기본으로 골라져 있어요」라고 말한다.
       그 말이 참이 되도록 여기서 차례를 맞춘다 — 손으로 적은 말과 목록이 갈라지지 않게. */
    const 미작성 = CAME.filter((d) => NOTE_ST[d.id] !== '작성완료')
      .slice()
      .sort((a, b) => (a.outAt ? 0 : 1) - (b.outAt ? 0 : 1) || String(a.outAt || '').localeCompare(String(b.outAt || '')));
...
<div class="filters">
  ${select(['최근 7일', '2026년 8월', '2026년 7월'], 0,
    { attr: ' data-filter-sel="hist" aria-label="기간"', vals: ['최근7일', '2026-08', '2026-07'] })}
  ${input({ ph: '반려견 이름으로 찾기', cls: 'search', attr: ' data-search-for="hist"' })}
</div>

<p class="t-sub mb4"><b data-filter-cnt="hist">${이력.length}</b>건 · 실패 <b class="dan" data-cnt-tag-for="hist" data-cnt-tag="실패">${이력.filter((r) => r.st === '실패').length}</b>건</p>

<div data-filter-list="hist" data-per-page="6">
${table(
      ['발송 시각', '반려견', '보호자', { t: '결과', cls: 'c' }, { t: '읽은 시각', cls: 'c' }, '미리보기', { t: '', cls: 'c' }],
      이력.map((r) => ({
        attr: ` data-tag="${r.st} 최근7일 ${r.t.slice(0, 2) === '08' ? '2026-08' : '2026-07'}"`,
        cls: r.st === '실패' ? 'bad' : '',
        cells: [
...
${modal('mPreview', '보낸 알림장 미리보기', `
  ...
  <div class="mt4">${gal(NOTES[0].pics, 'modal')}</div>
