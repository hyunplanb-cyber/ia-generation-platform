/* 07_AI빌드_스펙팩.json 의 136개 화면을 정적 HTML 로 생성한다.
...
const PRESET = path.resolve(ROOT, '스펙팩/가이드_01_코럴선셋.json');
...
for (const s of spec.screens) {
  ...
  const fn = BUILDERS[s.pageId] || BUILDERS[baseId];
...
<header class="gnb"><div class="gnb-in">
  ...
  <nav class="gnb-nav"><a href="pages/HO0101.html">고객 홈</a><a href="pages/PT0101.html">매장 홈</a></nav>
</div></header>
...
<section class="idx-hero"><div class="wrap">
  ...
  <div class="row mt8 wrap-row" style="gap:40px">
    ...
    <div><div class="t-sec" style="color:#fff">${baseCount}개</div><div class="t-sub" style="color:rgba(255,255,255,.85)">기본 화면</div></div>
    <div><div class="t-sec" style="color:#fff">${total - baseCount}개</div><div class="t-sub" style="color:rgba(255,255,255,.85)">상태·세부 화면</div></div>
...
<main class="main"><div class="wrap">
  <div class="card mb8"><div class="card-bd">
    <h2 class="t-card mb3">디자인 프리셋 — 가이드 01 코럴 선셋 × 레이아웃 A 사진 중심형</h2>
    ...
    <p class="t-sub mt2">레이아웃 A — 히어로는 사진 모자이크, 목록은 매거진 2열, 내비게이션은 2단 헤더, 상세는 전체 폭 갤러리 + 하단 고정 액션 바</p>
  </div></div>
  ...
  ${spec.menus.map((m) => `<section class="idx-menu">
    ...
    <div class="idx-list">
      ${m.screens.map((sc) => `<a class="${sc.pageId.endsWith('01') ? '' : 'sub'}" href="pages/${sc.pageId}.html">
        <span class="pid">${sc.pageId}</span><span>${sc.pageName}</span></a>`).join('')}
    </div></section>`).join('')}

  <div class="box mt8">
    <h3 class="t-card mb2">이 사이트에 대해</h3>
    <p class="t-sub">스펙팩(07_AI빌드_스펙팩.json)의 화면 정의에 디자인 프리셋 두 벌(가이드 01 코럴 선셋 · 레이아웃 A 사진 중심형)을 함께 넣어 만든 정적 프로토타입입니다.
