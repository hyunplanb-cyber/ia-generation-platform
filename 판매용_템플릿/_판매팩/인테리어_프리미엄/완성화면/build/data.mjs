 ...
 *
 * ⛔ 2026-09-02 — 같은 32평 전체 시공인데 화면마다 평당가가 달랐다.
 *      ES0101 견적 마법사                 75만원/평  (2,400만~3,100만)
 *      ES0201 · CT0101 · HO0301 머리말   101만원/평
 *      HO0301 «마감 등급별 차이»          210~340만원/평
 *   HO0301 은 «한 화면 안»에서 머리말이 101만원인데 바로 아래가 210만원이었다.
 *   그 화면 제목이 「값을 숨기지 않습니다」다.
 *
 * ⚠ 새 숫자를 지어내지 않았다. 팩이 이미 가진 FLAGSHIP(32평 · 전체 시공 · 고급)에서
 *   나눈다. 그러면 HO0101 의 평수 구간 넷이 모두 이 셈을 품는다 —
 ...
 *
 * ⚠ 옛 주석은 「FLAGSHIP 값을 마법사에 그대로 쓰면 안 된다 — 그건 확정 견적이다」라고
 *   했다. 그 걱정은 «숫자가 안 움직이던 때»의 것이다. 오늘 마법사가 평수·범위·등급·
 *   공간 넷을 다 보고 다시 세게 되었으므로, 이제는 잣대가 같아야 «고를 때마다 바뀐다»가
 *   참말이 된다. 둘이 다르면 손님은 같은 조건에서 두 값을 보게 된다. */
export const PRICING = {
  /* 32평 · 전체 시공 · 고급 = 1,012,500원/평 */
  평당고급: FLAGSHIP.totalFirst / FLAGSHIP.area,
  /* 실측 전이라 폭이 있다 — HO0301 머리말이 쓰던 그 폭 그대로. */
  폭: (FLAGSHIP.total + 1_500_000) / FLAGSHIP.totalFirst,
  범위배: { '전체 시공': 1, '부분 시공': 0.62 },
  /* 「기본」이 1 이다 — ES0201 의 data-base 가 그 기준이라 여기에 맞춘다. */
  등급배: { 기본: 1, 고급: 1.22, 프리미엄: 1.48 },
  기준등급: '고급',
  공간기준: 3,
  공간배: 0.06,
  /* 등급별 평당가. 화면에 「평당 OO만원」으로 적을 때 쓴다. */
  평당(등급) {
    return this.평당고급 * (this.등급배[등급] / this.등급배[this.기준등급]);
  },
  /* 조건을 다 넣었을 때의 배수. 마법사와 견적 결과가 «같은 셈»을 쓰게 한다. */
  배(o) {
    const 평 = Number(o.평) || FLAGSHIP.area;
    const 공간 = Number(o.공간) || this.공간기준;
    return (평 / FLAGSHIP.area) *
           (this.범위배[o.범위] || 1) *
           (this.등급배[o.등급 || this.기준등급] / this.등급배[this.기준등급]) *
           (1 + (공간 - this.공간기준) * this.공간배);
  },
};

/* 잣대가 팩의 다른 숫자와 어긋나면 여기서 멈춘다. */
{
  const 기준 = Math.round(PRICING.평당고급 * FLAGSHIP.area);
  if (기준 !== FLAGSHIP.totalFirst) {
    throw new Error(`PRICING.평당고급 × ${FLAGSHIP.area}평 = ${기준} 인데 totalFirst 는 ${FLAGSHIP.totalFirst} 입니다`);
  }
  /* HO0101 평수 구간이 이 셈을 품나 — 품지 못하면 홈과 견적이 다른 말을 한다. */
  for (const [이름, 평, 낮, 높] of [
    ['10평대', 15, 15_000_000, 22_000_000], ['20평대', 25, 24_000_000, 35_000_000],
    ['30평대', 32, 32_000_000, 46_000_000], ['40평대 이상', 45, 43_000_000, 62_000_000],
  ]) {
    const 최소 = PRICING.평당고급 * 평;
    const 최대 = 최소 * PRICING.폭;
    if (최소 < 낮 || 최대 > 높) {
      throw new Error(`HO0101 ${이름}(${낮}~${높}) 이 ${평}평 셈 ${Math.round(최소)}~${Math.round(최대)} 을 못 품습니다`);
    }
  }
}
