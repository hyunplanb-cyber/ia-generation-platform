/* 07_AI빌드_스펙팩.json 의 144개 화면을 정적 HTML 로 생성한다.
...
for (const s of spec.screens) {
  const baseId = s.pageId.slice(0, 4) + '01';
...
const baseCount = spec.screens.filter((s) => s.pageId.endsWith('01')).length;
...
<header class="gnb"><div class="gnb-in">
  ...
  <nav class="gnb-nav"><a href="pages/HO0101.html">홈 화면 보기</a></nav>
</div></header>

<section class="idx-hero"><div class="wrap">
  <h1 class="t-page" style="color:#fff">${spec.project.concept ? '여행 프리미엄 — 전체 화면' : ''}</h1>
  ...
  <div class="row mt6 wrap-row" style="gap:32px">
    ...
    <div><div class="t-sec" style="color:#fff">${baseCount}개</div><div class="t-sub" style="color:rgba(255,255,255,.8)">기본 화면</div></div>
    <div><div class="t-sec" style="color:#fff">${total - baseCount}개</div><div class="t-sub" style="color:rgba(255,255,255,.8)">상태·세부 화면</div></div>
...
<main class="main"><div class="wrap">
  <div class="card mb8"><div class="card-bd">
    <h2 class="t-card mb3">디자인 프리셋 — 01 모던 네이비</h2>
    <div class="row wrap-row" style="gap:8px">
      ${[['primary', '#2B4A8B'], ['primary-hover', '#1F3A73'], ['accent', '#FF7A30'], ['background', '#F5F7FA'],
    ['surface', '#FFFFFF'], ['text', '#16233F'], ['text-muted', '#5A6B8C'], ['border', '#DFE4EC'],
    ['success', '#0F7A52'], ['warning', '#B4761A'], ['danger', '#C0392B']]
    ...
    <p class="t-sub mt4">Pretendard · 카드 12px / 버튼·입력 8px / 배지 6px · 4px 배수 간격 · 그림자는 카드에만 (0 1px 3px rgba(22,35,63,.08))</p>
  </div></div>
  ...
  ${spec.menus.map((m) => `<section class="idx-menu">
    ...
    <div class="idx-list">
      ${m.screens.map((sc) => {
      const isBase = sc.pageId.endsWith('01');
      return `<a class="${isBase ? '' : 'sub'}" href="pages/${sc.pageId}.html">
        <span class="pid">${sc.pageId}</span><span>${sc.pageName}</span></a>`;
    }).join('')}
    </div></section>`).join('')}

  <div class="box mt8">
    <h3 class="t-card mb2">이 사이트에 대해</h3>
    <p class="t-sub">스펙팩(07_AI빌드_스펙팩.json)의 화면 정의와 디자인 프리셋(프리셋_01_모던네이비.json)을 그대로 반영해 만든 정적 프로토타입입니다.
    기본 화면은 스펙팩의 화면 프롬프트를, 상태·세부 화면은 상위 화면의 레이아웃을 유지한 채 해당 상태만 바뀐 모습으로 구성했습니다.
