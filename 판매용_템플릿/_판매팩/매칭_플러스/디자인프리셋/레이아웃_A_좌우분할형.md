...
| 자리 | 어떻게 |
| --- | --- |
| 첫 화면 위쪽 | 좌우 2단. 왼쪽에 큰 제목과 설명, 오른쪽에 폼 또는 대표 이미지. |
| 목록 화면 | 3열(1440px에서 카드 453px). 카드 안은 사진 없이 글자 위계로만 나눈다. 1024px에서 2열, 720px 아래로는 1열. |
| 내비게이션 | 상단 가로 GNB. 로고 왼쪽, 액션 버튼 오른쪽 끝. |
| 상세 화면 | 본문 한 단(최대 폭 760px) + 하단 고정 액션 바. |
| 카드(썸네일) | 가로로 넓은 사진 아래 제목 두 줄까지. 재생 시간·배지는 사진 위 우하단에. (비율 16:9) |

**어울리는 곳** — 설득이 먼저인 서비스(랜딩·구독·B2B 소개)

## 칸이 몇 개이고, 카드가 몇 px인가

| 화면 폭 | 좌우 여백 | 칸 사이 | 칸 수 | 카드 한 장 |
| --- | --- | --- | --- | --- |
| 1440px 이상 (노트북) | 24px | 16px | 3칸 | 453px |
| 1024px (태블릿 가로) | 24px | 16px | 2칸 | 480px |
| 375px (휴대폰) | 16px | 12px | 1칸 | 343px |

카드 사진은 **16:9**입니다. 위 폭에 맞추면 사진 크기는 이렇게 됩니다.

- 노트북 453×255 · 태블릿 480×270 · 휴대폰 343×193
...
이 뼈대의 기본 카드는 **와이드 카드**입니다. 아래 중 다른 것으로 바꿔 쓰셔도 됩니다 — 뼈대는 그대로 두고 카드만 갈아 끼우면 인상이 크게 달라집니다.

| 카드 모양 | 비율 | 어떻게 | 어울리는 곳 |
| --- | --- | --- | --- |
| 정사각 카드 | 1:1 | 사진이 카드 위쪽을 정사각으로 채우고, 아래에 제목 → 보조정보 → 가격. | 상품·공구처럼 물건 자체를 보여줄 때 |
| 와이드 카드 **(기본)** | 16:9 | 가로로 넓은 사진 아래 제목 두 줄까지. 재생 시간·배지는 사진 위 우하단에. | 강의·영상처럼 화면을 담는 것 |
...
- 내비게이션: 상단 가로 GNB. 로고 왼쪽, 액션 버튼 오른쪽 끝.
- 상세 화면: 본문 한 단(최대 폭 760px) + 하단 고정 액션 바.
- 카드(썸네일): 가로로 넓은 사진 아래 제목 두 줄까지. 재생 시간·배지는 사진 위 우하단에. 이미지 비율은 16:9 를 지켜줘.
