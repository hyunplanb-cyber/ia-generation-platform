...
export function PR0701(ctx) {
  ...
  ${picOnly ? `
    ...
    ${sec('', `<div class="g6">${list.flatMap((r) => Array.from({ length: r.pics }, (_, i) => `<button type="button" class="gph" title="${esc(r.who)}" data-toast="사진 크게 보기는 서버가 연결되면 열립니다">${ph(r.who + i, 'ph-11', '후기 사진', '800×800')}</button>`)).join('')}</div>`)}
