/* 07_AI빌드_스펙팩.json 의 2뎁스 화면(디럭스 · 37화면)을 정적 HTML 로 생성한다.
...
import { PAGES as ACDB } from './pages-ac-db.mjs';
import { PAGES as CU } from './pages-cu.mjs';
import { PAGES as BK } from './pages-bk.mjs';
import { PAGES as OR } from './pages-or.mjs';
import { PAGES as PD } from './pages-pd.mjs';
import { PAGES as MK } from './pages-mk.mjs';
import { PAGES as ST } from './pages-st.mjs';
import { PAGES as TMSE } from './pages-tm-se.mjs';
...
const LAYOUT = path.resolve(ROOT, '스펙팩/레이아웃_A_대시보드형.json');
const OUT = path.resolve(ROOT, 'pages');

const BUILDERS = { ...ACDB, ...CU, ...BK, ...OR, ...PD, ...MK, ...ST, ...TMSE };
...
<header class="gnb"><div class="gnb-in">
  ...
  <nav class="gnb-nav"><a href="pages/DB-01.html">대시보드</a><a href="pages/AC-01.html">로그인 화면</a></nav>
</div></header>

<section class="idx-hero"><div class="wrap">
  <h1 class="t-page">통합 비즈니스 관리자 시스템 — 전체 화면</h1>
  <p class="mt4" style="opacity:.94;max-width:860px">${spec.project.concept}</p>
  <div class="row mt8 wrap-row" style="gap:40px">
    <div><div class="t-sec">${total}개</div><div class="t-sub" style="color:rgba(255,255,255,.8)">전체 화면</div></div>
    <div><div class="t-sec">2뎁스</div><div class="t-sub" style="color:rgba(255,255,255,.8)">설계 깊이</div></div>
    <div><div class="t-sec">${spec.menus.length}개</div><div class="t-sub" style="color:rgba(255,255,255,.8)">메뉴 그룹</div></div>
  </div>
</div></section>

<main class="main"><div class="wrap">
  <div class="card mb8"><div class="card-bd">
    <h2 class="t-card mb4">디자인 프리셋 — 가이드 02 ${preset.preset.name} × 레이아웃 A ${layout.layout.name}</h2>
    ...
    <p class="t-sub mt2">레이아웃 A ${layout.layout.name} — ${layout.slots.nav} ${layout.slots.hero} ${layout.slots.list} ${layout.slots.detail}</p>
    <p class="t-sub mt2">카드 모양은 <b>${layout.card.name}</b> — ${layout.card.shape}
    백오피스는 숫자와 상태가 주인공이라 사진을 쓰지 않습니다.</p>
    <p class="t-sub mt2">간격은 밀도(컴팩트)에서 뽑은 눈금 12개를 CSS 값으로 박아 두었습니다 — 화면마다 눈대중으로 정하지 않습니다.</p>
  </div></div>

  <div class="box mb8" style="border-left:3px solid var(--primary)">
  ...
  <div class="box mt8">
    ...
    <p class="t-sub">스펙팩(07_AI빌드_스펙팩.json)의 화면 정의에 디자인 프리셋 두 벌(가이드 02 ${preset.preset.name} · 레이아웃 A ${layout.layout.name})을 함께 넣어 만든 정적 프로토타입입니다.
    화면마다 스펙팩에 적힌 화면 프롬프트를 그대로 따라 만들었습니다. 탭·상태·예외까지 펼친 3뎁스는 프리미엄 등급에 들어 있습니다.
    사진 자리는 실제 사진 대신 <b>무엇이 들어갈 자리인지와 권장 크기</b>를 적은 옅은 블록으로 두었습니다.
