...
| 자리 | 어떻게 |
| --- | --- |
| 첫 화면 위쪽 | 화면 폭을 꽉 채운 배경 위에 큰 검색바 1개(입력칸 2~3개 + 검색 버튼). 아래에 숫자 지표 3개. |
| 목록 화면 | 카드 그리드 3~4열. 카드마다 이미지 → 제목 → 보조정보 → 가격 순. |
| 내비게이션 | 상단 가로 GNB 한 줄. 로고 · 검색 · 메뉴 · 로그인. |
| 상세 화면 | 본문 왼쪽 + 오른쪽에 따라다니는 요약·액션 카드(sticky). |
| 카드(썸네일) | 사진이 카드 위쪽을 정사각으로 채우고, 아래에 제목 → 보조정보 → 가격. (비율 1:1) |

**어울리는 곳** — 예약·검색이 시작점인 서비스(숙소·강의·매장 찾기)
