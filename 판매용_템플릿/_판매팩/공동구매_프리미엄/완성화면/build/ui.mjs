...
/* 링크 목적지 — 화면 코드는 스펙팩 그대로 HO0101 형태다.
   화면 안에서는 짧게 'HO-01' 로 적고, 여기서 스펙팩 코드로 바꾼다.
   'HO-01' → 'HO0101' (그 묶음의 첫 화면). 없는 코드는 같은 메뉴 첫 화면으로 보낸다. */
let PAGES = null;
...
/* 레이아웃 A — 상단 가로 GNB 한 줄. 어두운 배경에 흰 글자,
   오른쪽 끝에 밝은 버튼 하나. 카테고리 줄(2단 헤더)은 쓰지 않는다. */
function gnb(activeId, o = {}) {
  ...
  const util = owner
    ? `<a class="btn btn-sm" href="${link('HO-01')}">참여자 화면 보기</a><a class="gnb-ava" href="${link('AC-01')}" title="내 정보">${phAva(30, 'me')}</a>`
    : `<a href="${link('CS-01')}">이용 안내</a><a class="btn btn-sm" href="${link('HM-01')}">진행자 화면으로</a><a class="gnb-ava" href="${link('AC-01')}" title="내 정보">${phAva(30, 'me')}</a>`;

  return `<header class="gnb dark">
  ${o.topbar || ''}
  <div class="gnb-in">
    <a class="logo" href="${link(owner ? 'HM-01' : 'HO-01')}"><span class="mark">${SITE.mark}</span>${SITE.name}${owner ? '<span class="badge" style="margin-left:6px;border-color:rgba(255,255,255,.45);color:#fff">진행자</span>' : ''}</a>
...
/* 지금 보는 화면이 그 메뉴 안에 있는지 — 상태 화면(HM0102)도 부모(HM0101)를 켠다 */
const sameGroup = (activeId, id) => String(toPageId(id)).slice(0, 4) === String(activeId).slice(0, 4);

/* 진행자 영역 사이드바 */
export const hostNav = (activeId) => {
  const groups = [
    ['공구', [['HM-01', '진행자 대시보드'], ['HS-01', '진행자 시작 안내'], ['HS-02', '공구 개설'], ['HS-03', '개설 검수'], ['HM-02', '참여 현황'], ['HM-03', '마감·성사 처리']]],
    ['배송', [['HM-04', '발주·배송 관리'], ['HM-05', '참여자 공지']]],
    ['정산', [['SE-01', '정산 내역'], ['SE-02', '계좌·세금 설정'], ['SE-03', '정산 상세']]],
  ];
  return `<aside class="side">${groups.map(([g, items]) => `<div class="gl">${g}</div>${items.map(([id, label]) =>
    `<a href="${link(id)}"${sameGroup(activeId, id) ? ' class="on"' : ''}>${label}</a>`).join('')}`).join('')}</aside>`;
};

export const myNav = (activeId) => {
  const items = [['MY-01', '내 공구함'], ['MY-02', '참여 상세'], ['RV-03', '관심·알림'], ['RV-01', '내 후기'], ['AC-02', '내 정보·배송지'], ['AC-03', '알림 설정'], ['CS-02', '1:1 문의']];
  return `<aside class="side"><div class="gl">내 정보</div>${items.map(([id, label]) =>
    `<a href="${link(id)}"${sameGroup(activeId, id) ? ' class="on"' : ''}>${label}</a>`).join('')}</aside>`;
};

/* ---------- 상태 화면 조각 ----------
   3뎁스 화면은 "이 상태일 때 무엇이 보이는가"를 보여 준다.
   토스트·모달·시트는 뜬 채로 박아 두고, 닫기를 누르면 실제로 사라진다. */
export const toastNow = (msg, o = {}) => `<div class="toast${o.ok ? ' toast-ok' : ''}">
  <span>${msg}</span><span class="act" data-close=".toast">${o.act || '닫기'}</span></div>`;

export const modalNow = (title, body, ft, o = {}) => `<div class="dim"><div class="modal ${o.cls || ''}">
  <div class="hd">${title}</div><div class="bd">${body}</div>
  ${ft ? `<div class="ft">${ft}</div>` : ''}</div></div>`;

export const sheetNow = (body) => `<div class="dim" style="align-items:flex-end;padding:0">
  <div class="sheet"><div class="grip"></div><div style="padding:0 var(--card-pad) var(--card-pad)">${body}</div></div></div>`;

/** 불러오는 중 — 카드 자리를 스켈레톤으로 채운다 */
export const skCard = () => `<div class="deal"><div class="sk sk-ph" style="aspect-ratio:1/1"></div>
  <div class="bd"><div class="sk sk-line" style="width:38%"></div><div class="sk sk-line" style="width:90%"></div>
  <div class="sk sk-line" style="width:64%"></div><div class="sk sk-line" style="width:100%;height:8px"></div>
  <div class="sk sk-line" style="width:52%;margin-bottom:0"></div></div></div>`;
export const skGrid = (n = 8, cls = 'stair') => `<div class="${cls}">${Array.from({ length: n }, skCard).join('')}</div>`;
export const skRows = (n = 4) => Array.from({ length: n }, () => `<div class="list-row">
  <div class="sk" style="width:104px;height:104px;flex:none;border-radius:var(--r-btn)"></div>
  <div class="grow"><div class="sk sk-line" style="width:22%"></div><div class="sk sk-line" style="width:70%;height:17px"></div>
  <div class="sk sk-line" style="width:44%"></div></div>
  <div class="sk sk-line" style="width:88px;height:22px"></div></div>`).join('');

/** 지금 어떤 상태인지 — 화면 맨 위에 한 줄로 알린다 */
export const spin = (msg) => `<div class="center" style="padding:var(--s8) 0">
  <div class="spinner"></div><p class="t-sub mt3">${msg}</p></div>`;
