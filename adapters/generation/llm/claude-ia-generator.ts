...
const SKELETON_SYSTEM = [
  ...
  // 예시만 늘어놓으면 모델이 절반만 따라 한다(실측: 예외 화면이 44화면 중 2개까지 떨어졌다, 2026-08-07).
  // 「몇 개 이상」이라는 셀 수 있는 하한선을 주면 모델이 스스로 세어 채운다.
  // 이 화면들이 우리가 파는 이유 그 자체라, 여기서 빠지면 상품이 성립하지 않는다.
  "- **「안 되는 길」 화면을 8개 이상 반드시 넣으세요(전체의 20% 이상).**",
  "  「안 되는 길」이란 일이 뜻대로 안 풀렸을 때 보이는 화면입니다: 결과 없음 / 비어 있음 / 입력 오류 / 결제 실패 /",
  "  품절 / 마감·종료 / 권한 없음 / 통신 오류 / 취소·거절.",
  "  AI 도구들은 이 화면을 빼먹습니다. 이 산출물의 가장 중요한 가치가 바로 이 화면들입니다.",
  "  화면명에 '없음 / 비어 있음 / 오류 / 실패 / 품절 / 마감 / 권한 없음' 같은 말이 그대로 들어가게 쓰세요.",
  // 하한선만 주면 「무엇을」 넣을지는 모델 마음이라 컨셉마다 들쭉날쭉했다.
  // 기능→예외를 1:1로 묶어 두면 컨셉이 달라도 훑을 목록이 같아져, 업종을 안 타고 고르게 나온다.
  "- **아래 체크리스트를 하나씩 확인하며, 해당하는 기능이 있으면 그 예외 화면을 반드시 목록에 넣으세요.**",
  "  · 목록/피드가 있다 → '(목록명) - 비어 있음'",
  "  · 검색이 있다 → '검색 결과 - 결과 없음'",
  "  · 입력 폼이 있다 → '(폼명) - 입력 오류'",
  "  · 로그인이 있다 → '로그인 - 실패'",
  "  · 결제/주문이 있다 → '결제 - 실패'",
  "  · 상품·재고·좌석이 있다 → '(대상) - 품절·마감'",
  "  · 신청·예약이 있다 → '신청 - 마감' 또는 '예약 - 취소·거절'",
  "  · 저장함·찜·장바구니가 있다 → '(대상) - 비어 있음'",
  "  · 회원 전용 기능이 있다 → '권한 없음'",
  "  해당하지 않는 항목만 건너뛰고, 해당하면 빠짐없이 넣으세요.",
  // 예외를 늘리라고만 했더니 총량이 50을 넘겨 버렸다(실측 56개). 상한을 못 박고,
  // 넘칠 때 무엇부터 덜어낼지까지 정해 준다 — 안 그러면 모델이 예외 화면부터 지운다.
  "- 메뉴당 화면 4~7개, **전체 화면 수는 30~50개**로 맞추세요. 50개를 넘기면 안 됩니다.",
  "  넘칠 것 같으면 **덜 중요한 정상 화면부터 덜어내고, 위의 「안 되는 길」 화면은 그대로 두세요.**",
...
const DETAIL_SKELETON_SYSTEM = [
  ...
  // 손님용 메뉴와 사장용 메뉴를 한 덩어리로 뽑아 놨더니, 만들어진 사이트에서 둘이
  // 한 헤더에 나란히 붙어 "이상한 사이트"가 됐다(2026-08-04).
  "- **쓰는 사람이 둘 이상이면(손님 / 운영자·판매자·관리자) 메뉴를 그 순서대로 몰아서 배치하세요.**",
  "  손님이 쓰는 메뉴를 앞에, 운영자가 쓰는 메뉴를 뒤에 모으고, 섞지 마세요.",
  "  운영자 메뉴의 nameKo에는 무엇을 관리하는지가 드러나게 하세요(예: `예약 관리`, `매장 운영`, `정산`).",
  "  둘은 사실상 다른 사이트라, 한 화면에 나란히 놓이면 서비스가 이상해 보입니다.",
  ...
  "- **전체 3뎁스 화면 수가 100~150개가 되도록** 촘촘히 뽑으세요(억지 중복이 아니라, 실제로 확인해야 할 상태들).",
...
const DETAIL_SYSTEM = [
  ...
  // 색을 프롬프트마다 박았더니, 사용자가 다른 디자인 프리셋을 골랐을 때 지시가 둘이 되어
  // AI 코딩 도구가 "충돌한다"며 멈췄다(2026-08-03). 프롬프트는 무엇을, 프리셋은 어떻게.
  "- **prompt에 색상·색상코드(#RRGGBB)·글꼴·톤 같은 '어떻게 보일지'는 쓰지 마세요.**",
  "  디자인은 별도의 디자인 프리셋이 정합니다. prompt는 '무엇을 만들지'(구성·순서·동작)만 씁니다.",
  "  '디자인 컨셉'이 주어져도 그대로 옮겨 적지 말고, 화면에 **무엇이 들어갈지**를 정하는 데만 참고하세요.",
  "- targetRef는 아래 '전체 화면 목록'에 있는 ref만 쓰세요(다른 메뉴의 화면으로 이동해도 됩니다).",
  // 이동을 아껴 정의했더니 화면 130개 중 43개에만 버튼이 붙어, 만들어진 사이트의 버튼
  // 절반이 눌러도 아무 일 없는 상태가 됐다(2026-08-03). 갈 곳이 있으면 적극적으로 잇는다.
  "- **화면마다 나가는 버튼을 되도록 1개 이상 정의하세요.** 목록→상세, 검색전→결과, 폼→완료,",
  "  상세→예약/구매, 빈 목록→둘러보기, 오류→다시 시도처럼 갈 곳이 분명한 이동은 빠짐없이 연결합니다.",
  "  funcDef에 적은 버튼·링크 중 이동이 생기는 것은 buttons에도 넣으세요.",
  "  정말 이동이 없는 화면(최종 완료 화면 등)만 빈 배열([])로 둡니다.",
