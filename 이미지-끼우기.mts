...
const 짝파일 = [...new Set(짝목록.map(([, f]) => f))].filter((f) => f !== "(비움)").map((f) => {
  const 방 = 어디있나(f);
...
for (const f of readdirSync(pages).filter((x) => x.endsWith(".html"))) {
  ...
  const 후 = 전.replace(
    ...
    (통째: string, 여는: string, 속: string, 닫는: string, 자리: number) => {
      ...
      /* ⚠ 자리표의 «제 이름»을 «옆 글»보다 «먼저» 본다 (2026-08-14).
         둘을 한 덩어리로 합쳐 놓았더니, 「짐 사진」 자리 뒤에 업체 이름이 따라온다는
         이유로 업체 그림이 들어갔다 — 표에서 위에 적힌 줄이 이겼기 때문이다.
         자리표가 스스로 「짐 사진」이라고 말하면 그 말이 옆 글보다 옳다. */
      const 옆글 = 얼굴인가(여는) ? "" : 전.slice(자리 + 통째.length, 자리 + 통째.length + 300).replace(/<[^>]+>/g, " ");
      const 짝 = (글 && 짝목록.find(([말]) => 글.includes(말))?.[1])
        ?? (옆글 && 짝목록.find(([말]) => 옆글.includes(말))?.[1]);
      /* 짝표에도 «(비움)» 을 쓸 수 있다 — 「짐 사진」처럼 손님이 찍어 올릴 자리를
         이름으로 콕 집어 비운다. 배치표의 역할로는 못 가른다: 「짐 사진」은 아무 규칙에도
         안 걸려 기본역할(시술)로 떨어지는데, 그 역할에는 채울 그림이 들어 있기 때문이다.
         2026-08-14 에 시술 역할을 직업 동물로 바꿨더니 짐 사진 9곳·후기 26곳이 같이 찼다. */
      if (짝 === "(비움)") return 통째;
      if (짝) {
