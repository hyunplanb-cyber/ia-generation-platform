...
import { Check, Sparkles, ListTree, Palette } from "lucide-react";
  ...
  return (
    <div className="flex flex-col gap-6">
      <FormHero
        ...
        icon={Palette}
        title="주요 메뉴 · 디자인 컨셉"
        description="떠오르는 메뉴 구성과 원하는 디자인 분위기를 알려주세요. 다 적었다면 오른쪽에서 바로 산출물을 생성할 수 있어요."
      />
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
        ...
        <aside className="lg:sticky lg:top-6 lg:self-start">
          {hasMenus ? (
            <div className="flex flex-col gap-2 rounded-2xl border border-border bg-muted/30 p-6">
              <div className="flex items-center gap-2">
                <Sparkles className="size-5 text-primary" />
                <h2 className="font-semibold text-foreground">이미 산출물이 생성됐어요</h2>
              </div>
              <p className="text-sm leading-relaxed text-muted-foreground">
                생성된 산출물은 상단 <b className="font-semibold text-foreground">생성 산출물</b> 탭에서
                확인·수정할 수 있어요. 메뉴를 직접 추가하거나 다시 생성하려면{" "}
                <Link
            ...
            <div className="flex flex-col gap-4 overflow-hidden rounded-2xl border border-primary/30 bg-gradient-to-br from-primary-soft/70 to-pastel-yellow/30 p-6">
