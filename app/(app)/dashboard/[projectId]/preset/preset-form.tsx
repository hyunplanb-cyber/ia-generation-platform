...
export function PresetForm({
  ...
  async function handleSave() {
    setBusy(true);
    try {
      await savePresetConfigAction(projectId, cfg);
      setSaved(true);
    } finally {
      setBusy(false);
    }
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-8 px-6 py-10">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          디자인 시스템 문서</b>를 만들어요. 고른 내용은 전체 다운로드에 함께 담깁니다.
        </p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_20rem]">
        ...
        <div className="flex flex-col gap-7">
          <Section title="큰 방향" hint="브리프에서 고른 컨셉이 기본으로 잡혀 있어요.">
            <div className="grid gap-2 sm:grid-cols-3">
              {DESIGN_OPTIONS.map((opt) => (
                <button
                  key={opt.key}
                  type="button"
                  onClick={() => set("style", opt.key as DesignKey)}
          ...
          <Section title="주 색상" hint="버튼·강조에 쓰는 대표 색이에요.">
        ...
        <div className="lg:sticky lg:top-6 lg:h-fit">
          ...
          >
            ...
            <p style={{ color: pv.muted, fontSize: 13, lineHeight: 1.6, margin: 0 }}>
              고른 색과 모서리가 이렇게 적용돼요. 실제 산출물 문서에는 색 단계·타이포·간격까지 자세히 정리됩니다.
            </p>
          ...
          >
            ...
            {busy ? "저장 중" : saved ? "저장됨" : "프리셋 저장"}
          </button>
          <p className="mt-2 text-center text-xs text-muted-foreground">
            저장은 무료예요. 문서 파일은 <b className="text-foreground">전체 다운로드</b>에 포함됩니다.
          </p>
        </div>
      </div>
    </div>
