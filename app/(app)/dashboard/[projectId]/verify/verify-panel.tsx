...
import { VERIFY_CHUNK } from "@/lib/credits";
...
export function VerifyPanel({
  ...
  basicCost: number;
  detailCost: number;
  /** 이 프로젝트의 화면 수 — 값의 근거를 손님에게 그대로 보여준다. */
  screenCount: number;
  /** 상세로 볼 때 나눠 보는 묶음 수(화면 10개당 1묶음). */
  detailChunks: number;
  downloadCost: number;
  creditsOpen: boolean;
  children: ReactNode;
        ...
        <div className="lg:sticky lg:top-6 lg:h-fit">
          {done ? (
            ...
            <>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                생성된 산출물을 바탕으로 만들어요. <b className="text-foreground">주요</b>는 핵심 화면 위주로
                빠르게, <b className="text-foreground">상세</b>는 이 프로젝트의 화면 {screenCount}개를 하나도
                빠뜨리지 않고 봅니다.
              </p>
              <form action={formAction} className="mt-4 flex flex-col gap-2">
                ...
                >
                  ...
                  <span className="text-[11px] opacity-80">핵심 화면 위주 · 1묶음</span>
                </button>
                <button
                  type="submit"
                  name="mode"
                ...
                >
                  ...
                  <span className="text-[11px] opacity-80">
                    화면 {screenCount}개 전부 · {detailChunks}묶음
                  </span>
                </button>
              </form>
              <p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">
                상세는 화면 {VERIFY_CHUNK.screens}개씩 나눠 보고, 값도 그 묶음 수에 맞춰 받아요. 화면이
                많을수록 확인할 것도 많아지니까요. 만들다 실패한 묶음이 있으면 그만큼은 받지 않습니다.
              </p>
              <ul className="mt-3 flex flex-col gap-1 text-[11px] leading-relaxed text-muted-foreground">
