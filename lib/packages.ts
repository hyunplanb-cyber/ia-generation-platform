 ...
 * 계산하기 시작한다.
 *
 * ⚠ 「대신 홈에서 넷을 한 줄에 나란히 놓지 않는다」고 여기 적혀 있었는데, 2026-08-26
 *   홈 리뉴얼 시안이 넷을 한 줄에 놓았고 구매 목록·상세도 거기에 맞췄다(사장님 지시).
 *   틀린 말이 남아 있으면 다음 사람이 그걸 지키려다 화면을 되돌린다 — 그래서 지운다.
 *   이름을 사다리처럼 두는 것 자체는 그대로다.
 *
 ...
 * 2026-08-19 — 공연 티켓 파트너 관리자(ticket)를 만들었으나 진열하지 않았다.
 *   문서는 다 나왔지만(46/228) 완성화면(HTML)이 없어 «약속과 실물이 다른» 상태였다.
 * 2026-08-20 — **공연 티켓을 통째로 들어냈다.** 사장님 말씀 — 「공연은 판매할 거 아니야」.
 *   진열만 미루는 것과 «안 판다»는 것은 다르다. 안 팔 것을 코드에 남겨 두면
 *   다음 사람이 「이건 왜 진열이 안 되지」부터 다시 좇게 된다.
