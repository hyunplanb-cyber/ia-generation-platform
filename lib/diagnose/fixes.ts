 ...
 * 그래서 이때는 처방을 통째로 갈아 끼운다. 고칠 것은 하나뿐이다 — 서버 렌더링.
 */
...
function 안보임(id: string, label: string, c: FixContext): Fix {
  return {
    what: `0 으로 잡혔지만 «없다»는 뜻이 아닙니다. 이 사이트는 자바스크립트로 그려서 서버가 내주는 원본이 ${c.textLength}자뿐입니다. 화면에 잘 적어 두셨더라도 AI 봇은 그것을 받지 못합니다.`,
    // 조사를 붙이면 「통계·숫자을(를)」처럼 어색해진다. 낱말을 문장 끝에 두지 않는다.
    how:
      `새로 쓰기 전에 «서버 렌더링»부터 고치세요 — 지금 0 으로 잡힌 것은 ${label} 자체가 아니라 «로봇이 받는 화면»입니다. ` +
      `그것만 고치면 이미 써 두신 내용이 그대로 살아납니다. ` +
      `내용을 더 채우는 것은 그다음 일입니다. 지금은 무엇을 넣어도 로봇에게 안 갑니다.`,
