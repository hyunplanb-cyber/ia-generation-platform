/* 프리셋 팩을 «글자»로 만든다 — 파일로 쓰지 않는다.
 *
 * 왜 여기 있나 (2026-08-14)
 *   전에는 build-design-presets.mts 안에만 있었다. 그래서 브라우저에서 도는
 *   다운로드(zip-all-button.tsx)가 이 함수들을 못 불렀고, 손님이 받는 zip 에는
 *   디자인시스템 md «한 장»만 들어갔다. 판매팩은 11개인데.
 *
 *   ⚠ **파는 팩과 받는 팩은 반드시 같은 곳에서 나와야 한다.**
 *     두 곳에 적으면 갈린다 — lib/design-presets.ts 주석이 이미 그렇게 경고했다.
 *
 *   파일로 쓰는 일(node:fs)은 부르는 쪽이 한다. 여기는 순수하게 글자만 만든다.
 *   그래야 브라우저에서도 돈다.
 */
import {
...
const PRESETS: Preset[] = [
  {
    /* 2026-08-10 에 뒤늦게 넣었다. lib/design-presets.ts 는 처음부터 6색을 내놓는데
       이 목록에는 5색뿐이어서, 레트로를 고른 업종이 굽다가 죽었다.
       고르라고 내놓고 못 만드는 것은 없는 것보다 나쁘다 —
       고른 사람이 «다 골라 놓고 나서» 알게 되기 때문이다.
       색·크기·모서리는 전부 lib/design-presets.ts 의 retro 에서 그대로 옮겼다. */
    key: "retro",
    no: "01",
    name: "레트로 페이퍼",
    ...
    radius: { card: "10px", button: "8px", input: "8px", badge: "6px" },
    ...
    comp: [
      ...
      ["배지", "틸 강조는 원색 글자를 그대로 쓴다. 배경은 10% 농도, radius 6px"],
      ["표", "헤더 배경 진한 크림, 행 구분 border 1px, 행 높이 46px"],
      ["사이드바", "크림 배경, 활성 항목만 오렌지 10% 배경 + 오렌지 글자"],
    ],
    screens: [
      ["대시보드", "지표 카드 4개를 한 줄에. 숫자 34px/800, 라벨 13px muted"],
      ["목록", "카드 격자 우선. 필터는 상단 칩 줄, 카드 hover 시 border 진해짐"],
  ...
  },
  {
    ...
    c: {
      /* ⛔ 여기도 갈려 있었다(2026-09-07 pack-qa-spec-b). #15803D 는 제 페이지 배경
         #F0EFEB 위 4.36 · 팩이 쓰는 배지 자리(--pri-10 위) 4.39 로 4.5 에 미달이다.
         2026-09-03 대비 쓸어내기가 장비렌탈_디럭스 완성화면을 #147A3A(4.71 · 4.74)로
         내렸는데 손님이 받는 이 가이드만 옛 값이었다. 아래 text-muted 와 같은 자리다. */
      ...
      "border (구분선)": "#DBE7DC",
      /* 셋도 같은 쓸어내기에서 내려갔다 — 옛 값은 제 배경(#F0EFEB) 위에서
         4.36 · 4.28 · 5.62 였고 앞의 둘이 미달이었다. 새 값은 4.77 · 6.72 · 6.09 다.
         danger 는 옛 값도 넘겼지만 화면과 가이드가 갈리지 않게 함께 맞춘다. */
      success: "#14793A",
      warning: "#774806",
      danger: "#AF1B1B",
    },
...
export type { Preset };
export { PRESETS, md as buildGuideMarkdown, tokens as buildGuideTokens,
         layoutMd as buildLayoutMarkdown, layoutTokens as buildLayoutTokens, buildPreviewHtml };

/** 팩에 들어갈 열한 개 파일을 «이름과 내용»으로 만든다. 파일 쓰기는 부르는 쪽 몫이다. */
export function buildPresetPack(opts: {
  styles: string[];
  layouts: string[];
  fits?: string[];
}): { name: string; text: string }[] {
  const chosen: Preset[] = opts.styles.map((key, i) => {
    const found = PRESETS.find((x) => x.key === key);
    if (!found) throw new Error(`프리셋 정의가 없어요: ${key}`);
    return { ...found, no: String(i + 1).padStart(2, "0"), fits: opts.fits?.[i] ?? found.fits };
  });
  const chosenLayouts = opts.layouts.map((key) => {
    const found = LAYOUTS.find((l) => l.key === key);
    if (!found) throw new Error(`레이아웃 정의가 없어요: ${key}`);
    return found;
  });

  const 것들: { name: string; text: string }[] = [];
  for (const p of chosen) {
    const base = `가이드_${p.no}_${p.name.replace(/\s/g, "")}`;
    것들.push({ name: `${base}.md`, text: md(p) });
    것들.push({ name: `${base}.json`, text: JSON.stringify(tokens(p), null, 2) });
  }
  chosenLayouts.forEach((l, i) => {
    const no = String.fromCharCode(65 + i);   // A, B
    const base = `레이아웃_${no}_${l.label.replace(/\s/g, "")}`;
    것들.push({ name: `${base}.md`, text: layoutMd(l, no) });
    것들.push({ name: `${base}.json`, text: JSON.stringify(layoutTokens(l, no), null, 2) });
  });
  것들.push({ name: "프리셋_미리보기.html", text: buildPreviewHtml(chosen, chosenLayouts) });
  return 것들;
}
