...
const COMMON_CHECKS: { item: string; how: string; expect: string }[] = [
  ...
  /* ⭐ 여기부터는 스펙팩 §7-0 의 «② 기계가 보는 검수»·§7-9-2 와 «짝»이다 (2026-08-25).
     ...
     나누면서 실어 보낸 «파일만 보고 아는» 쪽에는 확인할 칸이 하나도 없었다.
     이 파일이 스스로 적어 둔 원칙 그대로다 — 「시킨 것과 보는 것이 같아야 한다」.
     ⚠ 스펙팩 7-9-2 의 검사 글을 고치면 여기도 같이 고친다. 둘은 한 벌이다. */
  {
    item: "끊어진 링크 — 갈 곳이 실제로 있는지",
    how: "스펙팩 7-9-2 의 검사 글을 사이트 «뿌리»에서 한 번 돌린다. ⚠ pages/ 만 주면 한 층 위의 화면 목록을 못 봐서 멀쩡한 화면이 외톨이로 잘못 잡힌다.",
    expect: "끊어진 링크 0건. `href` 가 가리키는 .html 이 모두 실제로 있다.",
  },
  {
    item: "없는 그림 · 빈 파일",
    how: "같은 검사 글의 결과에서 «없는 그림»과 «빈 파일» 줄을 본다.",
    expect: "둘 다 0건. `src` 가 가리키는 파일이 모두 있고, 0바이트로 남은 화면이 없다.",
  },
  {
    item: "외톨이 화면 — 아무 데서도 안 이어지는 화면",
    how: "같은 검사 글의 «외톨이» 줄을 본다. 나온 화면은 어디서 이어져야 맞는지 정하고 그 링크를 놓는다.",
    expect: "외톨이 0건. 만들어 놓고 갈 길이 없는 화면이 없다.",
  },
  {
    item: "화면 수가 약속한 수와 같은지",
    how: "만든 .html 수를 세어 스펙팩 4장의 화면 수와 견준다.",
    expect: "두 수가 같다. 목록에는 있는데 안 만든 화면, 만들었는데 목록에 없는 화면 둘 다 0.",
  },
  {
    item: "글자 대비 — 색을 고른 뒤 반드시 잰다",
    /* ⚠ 「7-9-2 가 가리키는 대로」로만 적어 두면 한 다리 건너서야 닿는다 — 대비를 재는
       글은 7-9 에 있고 7-9-2 는 파일만 본다. 재는 글을 «이름으로» 가리킨다 (2026-09-07). */
    how: "스펙팩 7-9 의 글을 화면에 붙여 넣으면 ⑧번이 잰다(7-9-2 는 파일만 보므로 대비를 재지 않는다). ⚠ 반투명 배경(10% 배지 등)은 «쌓아서» 진짜 바탕색을 구한 뒤 재야 한다 — 알파를 빼고 재면 멀쩡한 배지가 미달로 잡힌다.",
    expect: "본문 4.5 이상, 24px 이상이거나 굵은 18.66px 이상은 3.0 이상. 미달 0건.",
  },
  {
    item: "굳은 날짜 — 견본 날짜가 오늘에서 거꾸로 잡혔는지",
    how: "목록·상세의 날짜와 요일을 오늘 기준으로 다시 센다. 요일은 눈으로 세지 말고 달력과 맞춘다.",
    expect: "지난 날짜가 「예정」으로 보이지 않는다. 요일이 그 날짜의 실제 요일과 같다.",
  },
];
