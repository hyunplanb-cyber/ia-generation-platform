...
export function buildSpecPackMarkdown(
  ...
  /* ⛔ 2026-09-08 — 사장님이 이 스펙팩으로 사이트를 실제로 지어 보시고
     「콘텐츠 영역 엉망, 그리드 하나도 안 맞는 박스들하며. 이걸 돈주고 누가 사니」라고 하셨다.
     재 보니 41쪽 중 8쪽에서 한 화면 안 콘텐츠 왼쪽 끝이 최대 «일곱 번» 움직였고,
     요금 카드 셋 중 가운데 하나에만 배지가 붙어 제목이 38px(다른 쪽은 406px) 어긋나 있었다.

     위 두 줄(콘텐츠 영역·읽기 폭)은 «이미» 그러지 말라고 시켜 두었다. 그런데도 어긋났다.
     까닭은 «어기면 어떤 모습이 되는지»와 «그래서 무엇을 하라는지»가 없었기 때문이다.
     규칙만 적으면 안 지켜진다 — 어긴 모습과 지키는 방법을 같이 적는다.
     ⚠ 이 절을 지웠는지는 `11_내사이트_검수하는_법` 의 「폭」·「줄맞춤」 검사가 잡는다. */
  lines.push("### 2-1. 폭과 줄맞춤 — 여기서 제일 많이 어긋납니다");
  lines.push("");
  lines.push(
    "**한 화면 안에서 폭을 갈아타지 마세요.** 위 두 폭(콘텐츠 영역 · 읽기 폭)은 " +
      "**한 화면에 최대 두 가지**입니다. 글이 주인공인 덩어리만 읽기 폭으로 좁히고, 나머지는 전부 콘텐츠 영역 폭입니다.",
  );
  lines.push("");
  lines.push("| 이 덩어리는 | 어느 폭 |");
  lines.push("| --- | --- |");
  lines.push("| 카드 격자 · 표 · 목록 · 조작 위젯(슬라이더·계산기·필터) | **콘텐츠 영역 폭** |");
  lines.push("| 상세 본문 · 안내문 · 약관 · 긴 설명 | **읽기 폭** |");
  lines.push("");
  lines.push(
    "⛔ **거르는 것과 걸러지는 것은 같은 폭을 씁니다.** 검색 필터가 읽기 폭인데 그 아래 표가 " +
      "콘텐츠 폭이면, 필터가 그 표를 조작한다는 것이 눈으로 안 읽힙니다.",
  );
  lines.push("");
  lines.push(
    "**확인하는 법** — 화면을 위에서 아래로 훑으며 **덩어리들의 왼쪽 끝**을 봅니다. " +
      "왼쪽 끝이 **세 번 넘게 움직이면** 폭을 갈아타고 있는 것입니다. 값이 두 가지만 나와야 합니다.",
  );
  lines.push("");
  lines.push(
    "**한 줄에 나란히 서는 칸들은 «속 구조»를 같게 만드세요.** 한 칸에만 배지·꼬리표가 붙으면 " +
      "그 칸의 제목과 값만 아래로 밀려 층이 집니다. 배지가 없는 칸에도 **같은 높이의 빈 배지 줄**을 넣어 시작 높이를 맞춥니다.",
  );
  lines.push("");
  lines.push(
    "**카드 높이도 맞춥니다.** 격자의 «칸»은 늘어나도 그 안의 «카드»는 안 늘어납니다. " +
      "카드에 `height:100%`(또는 `align-items:stretch` + 세로 flex)를 주어 한 줄의 카드 바닥이 같이 떨어지게 하세요.",
  );
  lines.push("");
  lines.push(
    "**한 줄에 몇 칸을 놓을지는 이 표를 따릅니다.** 화면마다 다르게 정하면 같은 카드가 쪽마다 다른 폭으로 나옵니다.",
  );
  lines.push("");
  lines.push("| 화면 폭 | 카드 격자 | 지표·요약 칸 |");
  lines.push("| --- | --- | --- |");
  lines.push("| 1200px 이상 | 3칸 (사진 위주면 4칸) | 4칸 |");
  lines.push("| 900 ~ 1199px | 2칸 | 2칸 |");
  lines.push("| 900px 미만 | 1칸 | 2칸 |");
  lines.push("");
  lines.push(
    "⚠ **칸이 남으면 늘리지 말고, 격자를 쓰지 마세요.** 3칸 격자에 항목이 하나면 " +
      "그 하나가 왼쪽 3분의 1에만 놓이고 옆이 텅 빕니다.",
  );
  lines.push("");
  lines.push(
    "**간격도 화면마다 새로 정하지 않습니다.** 헤더 아래 첫 덩어리까지 · 덩어리 사이 · 카드 사이 — " +
      "이 세 값을 먼저 정하고 모든 화면에 같은 값을 씁니다. 화면마다 손으로 숫자를 적으면 쪽마다 다른 리듬이 됩니다.",
  );
  lines.push("");
  ...
  lines.push("아래 **7-9** 의 글을 화면에 붙여 넣으면 그 가운데 **열두 가지(②③④⑤⑨⑩⑪⑫⑬⑭ + 폭·줄맞춤)** 를 기계가 대신 재 줍니다.");
  ...
  lines.push("이 글이 대신 재 주는 것은 **열두 가지**입니다 — 나머지는 눈으로 보셔야 합니다.");
  ...
  lines.push("");
  /* ⚠ 7-0 의 «② 기계가 보는 검수» 는 「어디서: 아래 7-9-2」라고 보내면서 그 갈래의
     첫 줄에 «글자 대비»를 적어 두었다. 그런데 아래 글은 대비를 재지 않는다 — 파일만
     봐서는 실제로 칠해진 색을 알 수 없어 7-9 가 ⑧번으로 잰다. 그 안내가 7-9-3 끝에만
     있어서, 7-0 을 보고 여기로 온 사람은 대비를 찾다가 못 찾고 «안 재는 것»으로 읽는다.
     읽는 이가 «닿는 자리»에 적어 둔다 (2026-09-07). */
  lines.push("⭐ **글자 대비는 아래 글이 아니라 `7-9` 의 글이 ⑧번으로 잽니다.**");
  lines.push("파일만 봐서는 실제로 칠해진 색을 알 수 없어서입니다. 7-0 의 «② 기계가 보는 검수» 에서");
  lines.push("여기로 오셨다면, 대비만은 **7-9** 를 함께 돌리세요.");
  lines.push("");
  lines.push("```js");
  lines.push(파일검수글);
  lines.push("```");
