...
export function buildSpecPackMarkdown(
  ...
  lines.push(
    "  · **전체 화면 목록으로 돌아가는 길**을 **146개 화면 «전부»에** 두세요 — 오른쪽 아래에 떠 있는 `화면 목록` 단추 하나면 됩니다. 누르면 `화면목록.html` 로 갑니다. 접힌 패널 안에 `화면 정보` 같은 이름으로 넣으면 아무도 못 찾습니다.",
  );
  lines.push(
  ...
  lines.push("- **`index.html` 은 홈 화면입니다.** 화면 목록이 아닙니다 — 목록은 `화면목록.html` 로 따로 만듭니다.");
  lines.push(
    "- **`화면목록.html` 에는 화면을 «하나도 빼지 않고» 넣습니다.** 메뉴별로 묶고, 화면 ID 와 이름을 함께 적습니다. " +
    "이건 손님용이 아니라 **우리가 검수할 때 쓰는 화면**이라, 여기서 아무 화면이나 바로 열 수 있어야 합니다.",
  );
  lines.push("- 이 둘은 `완성화면/` 바로 아래 둡니다. 거기서는 `assets/...` 그대로 부르면 됩니다.");
