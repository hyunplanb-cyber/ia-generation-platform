...
export function buildSpecPackMarkdown(
  ...
  if (nav) {
    ...
    lines.push(
      `  - 두 영역을 오가는 길은 **하나만** 둡니다 — 손님 화면 우측 상단에 운영자 영역으로 넘어가는 버튼 하나(예: \`${nav.owner[0] ?? "운영"} 화면으로\`), 운영자 화면 우측 상단에 \`고객 화면 보기\`. 한 계정이 두 권한을 가지며(크몽·숨고와 같은 방식), 아직 권한이 없으면 전환 대신 **신청 화면으로 보냅니다.**`,
    );
  } else {
