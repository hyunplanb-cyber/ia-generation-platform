...
export function buildSpecPackMarkdown(
  ...
  // 만들어진 사이트를 열어 보면 버튼 절반이 눌러도 아무 일이 없었다(2026-08-03).
  // 이동 대상이 없는 버튼(저장·삭제·전화 걸기…)을 그냥 <button>으로 두기 때문이다.
  // 프로토타입이라 진짜 동작은 못 하더라도, 눌렀을 때 반응은 있어야 "만들어졌다"고 느낀다.
  lines.push(
    "- **이동 대상이 없는 버튼도 눌리면 반응하게 만드세요.** 저장·삭제·전화 걸기처럼 갈 화면이 없는 동작은, 눌렀을 때 짧은 안내(토스트)를 띄우거나 화면 안 상태를 실제로 바꾸세요. 아무 반응 없는 버튼은 만들다 만 화면으로 보입니다.",
  );
  lines.push("### 화면 구현");
