...
export const 화면검수글 = String.raw`(() => {
  ...
  for (const 부모 of [본문, ...본문.querySelectorAll("*")]) {
    ...
    for (const x of 틈들) {
      ...
      const 짧은이름표 = function (el) {
        const t = (el.textContent || "").trim();
        if (!t || t.length > 30) return false;
        return ![...el.children].some(function (c) { return (c.textContent || "").trim(); });
      };
      const 부품 = "input, select, textarea, button, .chip, .slot";
      if (짧은이름표(x.앞) && x.뒤.matches &&
          (x.뒤.matches(부품) || (x.뒤.querySelector && x.뒤.querySelector(부품)))) continue;
