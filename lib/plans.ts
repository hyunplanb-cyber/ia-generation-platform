...
export const PLANS: Record<PlanId, PlanDef> = {
  free: {
    ...
    priceKrw: 0,
    canDownload: false,
    // 횟수 제한을 두지 않는다 — 실제 한도는 크레딧이 정한다.
    // 기능마다 "무료 N회"를 따로 두면 무료 제공량이 기능 수만큼 불어나고,
    // 나중에 크레딧으로 합칠 때 정책이 어긋난다. 비용이 드는 행위(생성·검수·프리셋)는
    // 전부 크레딧에서 빠지므로, 지급한 무료 크레딧(FREE_CREDITS) 안에서 무엇을 몇 번 하든 사용자가 정한다.
    projectLimit: null,
    verifyLimit: null,
