...
for (const { 이름: 팩, 뿌리 } of 팩들) {
  ...
  오늘.setHours(0, 0, 0, 0);
  const 날짜모음 = new Map();
  /* ⛔ 2026-09-01: 위 주석은 「오늘보다 앞선 것만 센다」고 적어 두었는데, 정작 d 를 만들어 놓고
     «아무 데도 안 쓰고» 버리고 있었다. 가장 늦은 날짜를 한 줄 찍을 뿐이라 지난 날짜는
     몇 개가 있든 조용히 지나갔다 — 머리말이 검사한다고 적힌 항목이 실은 빈 껍데기였다.
     ⚠ 일부러 FAIL 로 올리지 않는다. 후기·주문내역·지난 수업의 날짜는 «지나 있어야» 맞다.
     흠인 것은 「예약 완료」처럼 앞날을 약속하는 화면의 지난 날짜뿐인데, 그 둘을 파일만
     보고는 가릴 수 없다. 그래서 «세어서 보여 주고» 사람이 판단하게 둔다. */
  const 지난날짜 = new Set();
  for (const 쪽 of 쪽들) {
    const s = readFileSync(join(쪽방, 쪽), "utf8");
    /* ⚠ 2026-09-01: 그물이 «2026-08-03» 만 잡고 있었다. 그런데 팩이 실제로 쓰는 꼴은
       «2026.08.03» 이다 — 여행 프리미엄 한 팩만 봐도 점 229개 대 붙임표 28개였다.
       열에 아홉을 안 세면서 「날짜 4가지」라고 찍고 있었다. 둘 다 잡는다. */
    for (const m of s.matchAll(/(20\d\d)[-.](\d\d)[-.](\d\d)/g)) {
      const dt = new Date(Number(m[1]), Number(m[2]) - 1, Number(m[3]));
      const 키 = `${m[1]}-${m[2]}-${m[3]}`;
      날짜모음.set(키, (날짜모음.get(키) || 0) + 1);
      if (dt < 오늘) 지난날짜.add(키);
    }
  }
  ...
  /* 2026-08-24 고침 — 코럴 선셋·레트로 페이퍼 값이 lib/design-presets.ts 와 어긋나 있었다.
     ...
     2026-09-07 고침 — 내추럴 그린이 #15803d 에서 #147a3a 로 내려갔다. 옛 값은 제 페이지
     배경(#F0EFEB) 위 4.36 · 팩이 쓰는 배지 자리(--pri-10 위) 4.39 로 4.5 에 미달이라
     09-03 대비 쓸어내기가 장비렌탈_디럭스 완성화면을 먼저 내렸고, 이번에 뿌리와
     손님 가이드(lib/preset-pack.ts)를 거기에 맞췄다. 이 표도 따라온다. */
  const 이름색 = {
    "모던 네이비": "#2b4a8b", "미니멀 모노": "#111111", "코럴 선셋": "#e02a0e",
    "일렉트릭 바이올렛": "#5b4fe5", "내추럴 그린": "#147a3a", "레트로 페이퍼": "#bc5918",
  };
  ...
  결과.push({
    ...
    날짜종류: 날짜모음.size, 가장늦은날짜: 가장늦은, 지난날짜: 지난날짜.size,
    요일탈, 프리셋탈, 프리셋, 레이아웃,
  });
}
...
for (const r of 결과) {
  ...
  if (r.가장늦은날짜) {
    ...
    if (r.지난날짜)
      console.log(`  · 그 가운데 오늘(${((t) => [t.getFullYear(), String(t.getMonth() + 1).padStart(2, '0'), String(t.getDate()).padStart(2, '0')].join('-'))(new Date())})보다 지난 것 ${r.지난날짜}가지`
        + (r.날짜종류 === r.지난날짜 ? " — 앞날 날짜가 하나도 없다" : ""));
  }
