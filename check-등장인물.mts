...
const 모든파일 = 파일들(뿌리);
/* ① 은 원본을 «뺀다» — 「안 쓰는 말」 표가 옛말을 담고 있는 것이 당연하기 때문이다.
   ② 는 원본도 «본다». 2026-09-10 에 ② 까지 같이 빼 두는 바람에, 「여기가 한 곳뿐인
   원본이다」라고 적힌 파일이 그날 옮겨진 고양이 그림 자리를 가리키는데도 초록으로 지나갔다.
   원본이 틀리면 나머지가 다 틀린다. 원본이야말로 제일 먼저 재야 하는 파일이다. */
const 파일 = 모든파일.filter((f) => f !== 원본길);
let 흠 = 0;
...
console.log(`\n② 부르는 «자리»가 실제로 있나 — 파일 ${모든파일.length}개를 봅니다 (원본 포함)\n`);
