...
export const snsContent = pgTable(
  "sns_content",
  {
    ...
    /** ⭐ 카피·캐션·자막 «말고» 고칠 것 (2026-08-20 사장님).
     *
     *  글은 사장님이 이 화면에서 고치면 로컬이 그대로 가져간다(`글가져오기.mts`).
     *  그런데 「커버에 엉뚜한 그림이 들어갔다」·「상단 영상이 다른 편 것이다」 같은 것은
     *  글이 아니라 **굽는 쪽**을 손봐야 한다. 적을 자리가 없어 그동안 채팅으로만 오갔다.
     *
     *  ⛔ 여기에 뭔가 적혀 있으면 지킬이는 «다시 굽지 않는다».
     *     같은 재료로 다시 굽으면 같은 그림이 또 나온다 — 사람이 먼저 손봐야 한다. */
    fixNote: text("fix_note").notNull().default(""),
    /** 「그 밖에 고칠 것」에 적힌 것을 «굽는 쪽에서 손본» 때.
     *
     *  ⛔ 왜 따로 두나 (2026-08-26 사장님: 「그 밖에 고칠 것이 있는데 왜 지워?
     *     이것도 고쳐줘서 제작해 주는 거 아니야?」)
     *     맞는 말씀이다. 그동안 fix_note 한 칸이 두 가지 일을 했다 —
     *       ① 「이것 좀 고쳐 주세요」라는 «주문서»
     *       ② 지킴이를 멈추는 «스위치»
     *     그래서 푸는 길이 «주문서를 지우는 것»뿐이었다. 무엇을 부탁했는지 기록이
     *     통째로 사라지고, 사장님은 자기 말을 지워야 다음으로 갈 수 있었다.
     *
     *  ⭐ 둘을 가른다. 주문서(fix_note)는 «남는다». 굽는 쪽을 손본 사람이 검수 화면에서
     *     「손봤습니다 — 다시 구워 주세요」를 누르면 여기에 때가 찍히고 지킴이가 다시 돈다.
     *  ⚠ 주문서 «글이 바뀌면» 이 칸을 비운다 — 새 부탁은 아직 아무도 안 손본 것이다. */
    fixNoteDoneAt: timestamp("fix_note_done_at"),
    checkResult: text("check_result").notNull().default(""),
    /** 지킴이가 마지막으로 «막힌» 까닭. 잘되면 빈칸으로 지운다.
     *
     * 왜 필요한가 — 2026-08-25 사장님이 겪으셨다.
     *   「등록 중으로만 뜨고 멈춰 있는 것 같아. 지킴이 켠 상태로 요청했는데 안 올라와 있었어.」
     *   실제로는 지킴이가 돌았고, 유튜브 로그인이 만료(invalid_grant)돼 올리지 못하고
     *   상태를 그대로 둔 것이었다. 그런데 화면은 여전히 「등록 중」이라 —
     *   **아무 일도 안 일어나는데 일어나는 중인 것처럼 보였다.**
     *   지킴이 알림은 그 화면을 열어 둔 그때만 스쳐 지나가고 남지 않는다.
     *   그래서 «남는 자리»를 따로 둔다. */
    watcherError: text("watcher_error").notNull().default(""),
