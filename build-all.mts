...
const 업종들 = ["lms", "beauty", "travel", "matching", "groupbuy", "rental", "interior", "petcare", "used"] as const;
const 보류업종들 = ["admin"];
...
/* ── 7. 손님 화면에 올릴 그림 ────────────────────────────────
 *
 * ⛔ 2026-09-10 — 여기가 «비어 있었다». 그래서 반려동물케어·인테리어 상세에
 *   그림이 한 장도 안 떴다(반려동물케어 프리미엄: 그림 0장).
 *
 *   6번이 찍은 캡처를 `팩화면-웹용.mts` 가 webp 로 줄여 `public/pack-screens/` 와
 *   `lib/pack-screens.json` 에 넣어야 상세 화면이 그림을 본다. 그런데 그 한 걸음이
 *   «문서에만» 적혀 있었다 — `_작업/상세화면뜨기.mjs:112` 의 「이제 팩화면-웹용.mts 로
 *   webp 를 만듭니다」 한 줄. 부르는 코드는 저장소 어디에도 없었다.
 *
 *   6번 주석이 「글로 적어 두면 또 빠뜨리니 굽는 차례 안에 넣는다」라고 적어 두고도
 *   «그 다음 걸음»을 똑같이 글로 남겨서 똑같이 빠뜨렸다.
 *
 * ⚠ 업종을 안 대고 불러도 «항상» 돌린다. 크롬을 안 띄우고 이미 찍힌 png 만
 *   줄이는 일이라 빠르고, 한 업종만 구워도 목록 파일은 통째로 다시 써야 한다. */
console.log("\n7. 손님 화면에 올릴 그림 (webp)");
돌리기("팩 화면 webp", "팩화면-웹용.mts", []);
