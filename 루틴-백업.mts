...
if (무엇 === "재기") {
  ...
  if (!어긋남) {
    ...
    console.log(`\n  ⛔ ${셈}군데가 어긋나 있습니다.`);
    if (!살아있는것.size) {
      /* 살아 있는 쪽이 통째로 비었으면 «담을» 일이 아니다 — 새 컴퓨터다. 헷갈리게 두지 않는다. */
      console.log("     살아 있는 루틴이 하나도 없습니다 — 새 컴퓨터라면 되돌리세요:");
      console.log("       npx tsx 루틴-백업.mts --되돌리기");
    } else if (새것.length + 사라진것.length + 바뀐것.length) {
    ...
    /* ⚠ 시각표는 «되돌리기»로 못 고친다. 되돌리기는 SKILL.md 만 옮기고 등록은 안 건드린다.
       ...
       ⛔ 다만 살아 있는 쪽이 텅 비었을 때는 이 말을 하면 안 된다 — 바로 위에서
          「담지 말고 되돌리라」고 해 놓고 아래에서 「담으라」고 하면 서로 어긋난다. */
    if (시각표말.length && 살아있는것.size)
      console.log("     시각표는 살아 있는 등록이 언제나 옳습니다 — --저장 으로 담으세요.");
    console.log("");
    process.exit(1);
  }
} else if (무엇 === "저장") {
  /* ⛔⛔ 새 컴퓨터에서는 살아 있는 쪽이 «비어» 있다. 그때 --저장 을 누르면 사본이
     통째로 날아간다 — 백업하려다 백업을 지우는 것이다. 2026-08-22 되돌리기를
     시험하다 이 자리를 봤다. 아래에서 막는다.
     ⚠ 「비었을 때」만 막으면 부족하다. 열둘 중 열이 지워진 것도 사고일 가능성이 크다.
       절반 넘게 지우는 일은 «정말 지우겠다»고 말해야 한다. */
  const 지울셈 = 사라진것.length, 사본셈 = 저장된것.size;
  const 너무많이지움 = 사본셈 > 0 && 지울셈 > 0 && 지울셈 >= Math.max(2, Math.ceil(사본셈 / 2));
  if (!살아있는것.size && 사본셈) {
    console.log(`\n  ⛔ 살아 있는 루틴이 하나도 없습니다. 지금 담으면 사본 ${사본셈}개가 통째로 날아갑니다.`);
    console.log("     새 컴퓨터라면 담을 게 아니라 되돌려야 합니다:");
    console.log("       npx tsx 루틴-백업.mts --되돌리기\n");
    process.exit(1);
  }
  if (너무많이지움 && !인자.includes("--정말지운다")) {
    console.log(`\n  ⛔ 사본 ${사본셈}개 가운데 ${지울셈}개를 지우려 합니다 — 절반이 넘습니다.`);
    console.log(`     지워질 것: ${사라진것.join(" · ")}`);
    console.log("     정말 그럴 셈이면 깃발을 대세요:  npx tsx 루틴-백업.mts --저장 --정말지운다\n");
    process.exit(1);
  }
