...
function md(p: Preset): string {
  ...
  L.push("");
  L.push("### 격자 — 카드를 늘어놓는 자리");
  L.push("");
  L.push("카드·타일을 격자로 늘어놓을 때 **좌우와 위아래를 다른 값으로** 벌립니다.");
  L.push("");
  L.push("| 자리 | 값 |");
  L.push("| --- | --- |");
  L.push("| **카드와 카드 사이** (좌우) | 위 눈금의 「카드와 카드 사이 (가로)」 |");
  L.push("| **줄과 줄 사이** (위아래) | 위 눈금의 「덩어리와 덩어리 사이」 |");
  L.push("");
  L.push("**같은 값을 쓰면 줄이 안 갈립니다.** 카드에 상자가 있을 때는 테두리가 경계를 만들어 줬지만,");
  L.push("상자를 걷어낸 뒤로는 **위 카드의 가격과 아래 카드의 사진이 붙어** 한 덩어리로 읽힙니다.");
  L.push("위아래를 더 벌려야 \"여기까지가 한 줄\"이 보입니다.");
  L.push("");
  L.push("| 나누는 것 | 정하는 곳 |");
  L.push("| --- | --- |");
  L.push("| 몇 열로 나눌지 | 레이아웃 프리셋 |");
  L.push("| **사이를 얼마나 벌릴지** | **위 간격 눈금 (좌우·위아래 따로)** |");
  L.push("| 카드가 어떻게 생겼는지 | 카드 모양 |");
  L.push("");
  L.push("- **레이아웃 프리셋이 「넓게」라고 해도 값을 새로 만들지 마세요.** 레이아웃은 *몇 열로 무엇을 어디에*를");
  L.push("  말하는 층이고, *얼마나 벌릴지*는 이 눈금이 정합니다. 실제로 그 한 줄 때문에 40px을 쓴 격자가 있었고,");
  L.push("  같은 화면의 다른 격자는 16px이라 두 값이 나란히 놓였습니다.");
  L.push("- **격자 간격은 이름 붙인 값 하나에서 가져오세요.** 격자마다 숫자를 직접 고르면 한 사이트 안에서");
  L.push("  4~6가지로 갈라집니다 — 실제로 8·12·16·24·32·40px이 섞여 있었습니다.");
  L.push("- 히어로의 좌우 2단처럼 **카드 격자가 아닌 자리**는 이 규칙 밖입니다. 거기는 화면 뼈대라 레이아웃이 정합니다.");
  L.push("");
  L.push("### 박스형 콘텐츠 — 몇 개를 늘어놓고 사진은 어떤 비율로");
  L.push("");
  L.push("사진이 든 카드를 격자로 늘어놓을 때, **몇 개를 놓느냐가 사진 비율을 정합니다.**");
  L.push("");
  L.push("| 한 줄에 | 사진 비율 | 왜 |");
  L.push("| --- | --- | --- |");
  L.push("| **2개** | **4:3** | 칸이 넓으니 가로로 넓은 사진 |");
  L.push("| **3개** | **4:3** | 위와 같음 |");
  L.push("| **4개** | **1:1** | 칸이 좁다. 정사각이 가장 얌전하다 |");
  L.push("");
  L.push("4개짜리를 세로로 긴 3:4로 뒀더니 **카드 한 줄이 화면을 다 먹었습니다.** 칸이 좁을수록 세로로");
  L.push("길게 두고 싶어지지만, 그러면 한 줄만 보이고 다음 단락이 안 보입니다. 좁은 칸에는 정사각입니다.");
  L.push("");
  L.push("- **한 줄에 하나만 두지 않습니다.** 마지막 줄에도 하나만 남기지 마세요 —");
  L.push("  **항목 수를 열 수에 맞춰** 3열이면 3·6·9개, 4열이면 4·8개로 둡니다.");
  L.push("- **한 사이트에서 카드 사진 비율은 하나입니다.** 위 표의 짝(2·3열↔4:3, 4열↔1:1) 밖의 비율은 만들지 마세요.");
  L.push("  같은 홈에 세로 카드와 16:9가 섞여 있어서 **사는 사람이 사진을 두 벌 준비해야 했던** 적이 있습니다.");
  L.push("- **배너·차트·영상·프로필은 이 규칙 밖입니다.** 가로로 긴 배너·영상(16:9)·프로필(1:1)은 제 나름의 비율이 있습니다.");
  L.push("- 상품 사진처럼 **정사각이 업계 표준인 분야**는 1:1을 써도 됩니다. 다만 그때도 한 사이트 안에서는 하나로 갑니다.");
  L.push("- **가로 배너 한 장으로 한 단락을 채우지 마세요.** 배너는 위 표 밖이지만 \"한 줄에 하나만 두지 않는다\"는 그대로 걸립니다.");
  L.push("  배너 한 장이 있던 자리는 **두 장으로 나누고** 4:3으로 맞추세요.");
  L.push("- **가로로 넘기는 줄(캐러셀)도 몇 개 보이느냐로 셉니다.** 뒤에 더 있어도 눈에 4개가 보이면 4개 — 비율도 1:1입니다.");
  L.push("  카드 폭을 `260px`처럼 박아 두면 화면 폭이 바뀔 때 3.4개씩 어정쩡하게 걸치니, **칸 수로 나눠서** 정하세요.");
  L.push("");
  L.push("### 분리형과 혼합형 — 글자를 사진 아래 두느냐 위에 얹느냐");
  L.push("");
  L.push("박스형 콘텐츠는 두 가지입니다. **어느 쪽이든 위의 개수·비율 규칙은 똑같이 적용됩니다.**");
  L.push("");
  L.push("| | **분리형** | **혼합형** |");
  L.push("| --- | --- | --- |");
  L.push("| 생김새 | 사진 아래에 글자 | 사진 위에 글자를 얹음 |");
  L.push("| 담을 수 있는 것 | 제목·설명·평점·가격까지 여러 줄 | 제목 + 한 줄 정도 |");
  L.push("| 사진이 하는 일 | 무엇인지 알려 준다 | 분위기를 만든다 |");
  L.push("| 쓰는 자리 | 상품·강의 목록처럼 **고르는** 자리 | 특집·기획전처럼 **끌어당기는** 자리 |");
  L.push("");
  L.push("**분리형은 상자를 두르지 마세요.** 흰 배경에 테두리·그림자까지 두르면, 사진과 글자가 이미 한 덩어리인데");
  L.push("선을 한 번 더 치는 셈입니다. 격자로 늘어놓으면 그 선이 줄줄이 겹쳐 화면이 답답해집니다.");
  L.push("**모서리는 사진이 가집니다** — 상자를 없앤 자리에서 사진이 카드 노릇을 합니다.");
  L.push("(혼합형은 사진이 곧 배경이라 이 규칙 밖입니다.)");
  L.push("상자를 없애면 글자가 **흰색이 아니라 페이지 배경 위**에 놓입니다. 대비를 다시 재세요.");
  L.push("");
  L.push("### 가격 표기 — 낼 돈부터");
  L.push("");
  L.push("**판매가 · 정가 · 할인율** 순서로 한 줄에 세웁니다.");
  L.push("");
  L.push("| 자리 | 모양 | 크기 |");
  L.push("| --- | --- | --- |");
  L.push("| **판매가** (낼 돈) | 진한 기본 글자색, 굵게 | 18 (상세는 28) |");
  L.push("| **정가** | 회색 + 취소선 | 13 |");
  L.push("| **할인율** | **강조색 글자**, 굵게 | 14 |");
  L.push("");
  L.push("- **할인율을 앞에 두지 마세요.** 눈이 먼저 \"20%\"에 걸려 정작 얼마인지를 뒤늦게 찾습니다.");
  L.push("  사는 분이 알고 싶은 건 **낼 돈**입니다.");
  L.push("- **할인율은 배지가 아니라 색 글자입니다.** 배지로 두면 정가·판매가와 무게가 비슷해져 셋이 서로 다툽니다.");
  L.push("  배지 자리는 `입문`·`NEW` 같은 **꼬리표 몫**입니다.");
  L.push("- **셋의 크기를 벌려 두세요.** 비슷하면 무엇을 먼저 읽어야 할지 눈이 헤맵니다.");
  L.push("- **강조색 원색을 글자로 쓰지 마세요.** 주황·코랄 원색은 흰 배경에서 3:1을 못 넘깁니다.");
  L.push("  글자용으로 한 단계 어두운 색을 따로 두세요.");
  L.push("- 가격 묶음은 **한 곳에서 만들어 쓰세요.** 자리마다 손으로 짜면 순서가 갈립니다.");
  L.push("");
  L.push("혼합형을 쓸 때 반드시 지킬 것:");
  L.push("");
  L.push("- **어둠막을 까세요.** 사진 위에 흰 글자를 그냥 얹으면 사진의 밝은 자리에서 글자가 사라집니다.");
  L.push("  **사진은 사는 분이 바꿀 것**이라 우리가 밝기를 미리 알 수 없습니다.");
  L.push("  글자 쪽에서 반대편으로 옅어지는 어둠막을 항상 깔아 두면 어떤 사진이 와도 읽힙니다.");
  L.push("- **글자 대비 4.5 이상**을 지키세요. 가장 밝은 사진(흰색)이 왔다고 치고 재세요.");
  L.push("- **글자를 짧게 쓰세요.** 제목 한 줄 + 설명 한 줄까지입니다. 그보다 길면 사진을 다 덮어 혼합형인 의미가 없습니다.");
  L.push("- **가격·평점처럼 정확히 읽어야 하는 것은 얹지 마세요.** 사진 위에서는 숫자가 잘 안 읽힙니다.");
  L.push("- **한 화면에 두 가지를 섞어도 됩니다.** 위는 혼합형 특집, 아래는 분리형 목록 — 하는 일이 다르므로 어수선하지 않습니다.");
  L.push("");
  L.push("### 버튼이냐 링크냐");
  L.push("");
  L.push("**다른 화면으로 데려가면 링크, 이 화면 안에서 끝나면 버튼입니다.**");
  L.push("");
  L.push("| 하는 일 | 무엇으로 | 눌렀을 때 |");
  L.push("| --- | --- | --- |");
  L.push("| 다른 화면으로 간다 | 링크 `<a href>` | 그 화면이 열린다 |");
  L.push("| 이 화면 안에서 끝난다 | 버튼 `<button>` | 안내 문구가 뜬다 |");
  L.push("");
  L.push("- 필터 적용·정렬·검색·다음 차시·시간 지점 이동은 **화면이 바뀌지 않는 조작**입니다.");
  L.push("  링크로 만들면 **자기 자신으로 가는 링크**가 되어, 눌러도 같은 화면이 다시 열립니다.");
  L.push("  사는 분 눈에는 *\"눌렀는데 아무 일도 안 나네\"* 로 보입니다.");
  L.push("- **나란히 선 버튼끼리는 종류를 섞지 마세요.** 하나는 화면이 넘어가고 하나는 안 넘어가면");
  L.push("  규칙이 없어 보입니다.");
  L.push("");
  L.push("**같은 길을 두 번 두지 마세요 — 위 카테고리 줄과 왼쪽 LNB.**");
  L.push("둘이 같은 곳을 가리키면 위쪽이 두 줄로 두꺼워지고, 어느 쪽을 눌러야 할지 잠깐 멈칫합니다.");
  L.push("");
  L.push("| 상태 | 왼쪽 LNB | 위 카테고리 줄 |");
  L.push("| --- | --- | --- |");
  L.push("| 맨 위 (LNB 보임) | 보인다 | **접힌다** |");
  L.push("| 스크롤해서 LNB가 위로 사라짐 | 안 보인다 | **내려온다** |");
  L.push("| LNB가 아예 없는 화면 | — | **늘 보인다** |");
  L.push("");
  L.push("- **LNB가 있는 화면은 접힌 채로 시작하세요.** 펼쳐 두었다가 나중에 접으면 첫 화면에서 한 번 번쩍입니다.");
  L.push("- **LNB가 없는 화면에서는 절대 접지 마세요.** 접으면 갈 길이 통째로 사라집니다.");
  L.push("- **기준은 \"LNB가 붙박이 머리말 아래로 사라지는 순간\"입니다.** 머리말 높이만큼 위를 깎아서 재세요.");
  L.push("- 같은 규칙을 **LNB를 쓰는 모든 레이아웃에 똑같이** 거세요. 화면마다 다르면 위쪽 높이가 제멋대로 바뀌어 보입니다.");
  L.push("");
  L.push("- **견본으로 가는 버튼을 화면 안에 두지 마세요.** `결과가 없을 때` `대화가 없을 때는 이렇게 보여요`");
  L.push("  처럼 예외 화면을 보여주려고 넣은 버튼·단락은 진짜 사이트에 없는 것입니다.");
  L.push("  검색창 옆에 그런 버튼이 서 있으면 사는 분은 그게 기능인 줄 압니다.");
  L.push("  빈 화면·오류 화면은 **화면 목록에 따로 한 장**으로 두세요 — 거기서 눌러 보면 됩니다.");
  L.push("");
  L.push("### 지켜 주세요");
  L.push("");
