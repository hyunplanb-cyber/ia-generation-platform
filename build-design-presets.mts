...
const TARGETS = {
  ...
  petcare: {
    ...
    // 코럴은 강아지·알림장 사진이 주인공인 밝고 따뜻한 인상에 맞아 앞에 둔다.
    styles: ["coral", "mono", "navy"] as const,
    // 보호자 쪽(예약·알림장)은 여백 중심형, 원 운영 쪽(등하원·반편성·백신)은
    // 표와 지표가 많아 대시보드형. calm × console 짝은 아직 어느 업종도 안 쓰던 것이다.
    layouts: ["calm", "console"] as const,
    dir: "_작업/반려동물케어_유치원예약/디자인프리셋",
    fits: [
  ...
  used: {
    /* 쓰던 물건 사진이 주인공인데 사진 품질이 제각각이라 색을 절제한다.
       코럴은 동네 장터의 밝고 친근한 인상에 맞아 앞에 두고,
       레트로 페이퍼는 「중고·손때 묻은 물건」의 결과 맞아 세 번째다.
       ⚠ 바이올렛(pastel)은 안 쓴다 — 본문 대비 1.01 로 재서 폐기한 색이다. */
    styles: ["coral", "mono", "retro"] as const,
    /* 매물을 훑어보고 견주는 것이 첫 동작이라 목록 중심형(썸네일 왼쪽·정보 오른쪽 가로 행),
       신고 처리 대기열·안전결제 정산처럼 표와 지표가 많은 운영자 쪽이 대시보드형.
       목록 × 대시보드 짝은 아직 어느 업종도 안 쓰던 것이다. */
    layouts: ["list", "console"] as const,
    dir: "_작업/중고거래_회원장터/디자인프리셋",
    fits: [
  ...
  "petcare-deep": {
    styles: ["coral", "mono", "navy"] as const,
    layouts: ["calm", "console"] as const,
    dir: "_작업/반려동물케어_유치원예약_상세IA/디자인프리셋",
    fits: [
      "발랄하고 친근한 인상이 필요한 소형견 유치원, SNS로 알림장을 자랑하는 브랜드",
      "절제된 신뢰감이 필요한 미니멀한 시설, 프리미엄 소수정예 펫유치원",
      "체계적인 운영이 강점인 대형·프랜차이즈형 펫유치원, 안심 케어를 앞세우는 브랜드",
