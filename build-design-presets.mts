...
const TARGETS = {
  ...
  ticket: {
    // 주최사가 쓰는 백오피스라 표·숫자가 주인공이다. 정돈된 실무형 네이비를 앞에 둔다.
    // 좌석도와 정산표가 많아 색을 절제한 미니멀 모노가 두 번째,
    // 야외 페스티벌·지역 축제까지 아우르도록 내추럴 그린을 세 번째로 둔다.
    styles: ["navy", "mono", "forest"] as const,
    // 회차·예매 내역처럼 훑어보고 비교하는 표가 첫 화면의 주인공이라 목록 중심형,
    // 예매번호·좌석으로 찾아 들어가는 일이 잦아 검색 중심형이 뒤를 받친다.
    layouts: ["list", "search"] as const,
    dir: "_작업/공연티켓_파트너관리자/디자인프리셋",
    fits: [
