...
function md(p: Preset): string {
  ...
  L.push("| **3개** | **4:3** | 위와 같음 |");
  L.push("| **4개** | **3:4** | 칸이 좁으니 세로로 긴 사진이 제 몫을 한다 |");
  L.push("");
  L.push("- **한 줄에 하나만 두지 않습니다.** 마지막 줄에도 하나만 남기지 마세요 —");
  L.push("  **항목 수를 열 수에 맞춰** 3열이면 3·6·9개, 4열이면 4·8개로 둡니다.");
  L.push("- **한 사이트에서 카드 사진 비율은 하나입니다.** 위 표의 짝(2·3열↔4:3, 4열↔3:4) 밖의 비율은 만들지 마세요.");
  L.push("  같은 홈에 3:4와 16:9가 섞여 있어서 **사는 사람이 사진을 두 벌 준비해야 했던** 적이 있습니다.");
  ...
  L.push("- **가로로 넘기는 줄(캐러셀)도 몇 개 보이느냐로 셉니다.** 뒤에 더 있어도 눈에 4개가 보이면 4개 — 비율도 3:4입니다.");
